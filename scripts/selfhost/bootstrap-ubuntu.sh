...
MOS_SELFHOST_CONFIG="${MOS_SELFHOST_CONFIG:-}"
if [[ -n "${MOS_SELFHOST_CONFIG}" && -f "${MOS_SELFHOST_CONFIG}" ]]; then
  set -a
...
MOS_OWNER_NAME="${MOS_OWNER_NAME:-${INSTALL_OWNER_NAME:-}}"
MOS_OWNER_EMAIL="${MOS_OWNER_EMAIL:-${INSTALL_OWNER_EMAIL:-}}"
MOS_OWNER_PASSWORD="${MOS_OWNER_PASSWORD:-${INSTALL_OWNER_PASSWORD:-}}"
...
configure_installer_seed_env() {
  ...
  log "Applying installer owner details before stack env generation"
...
reconcile_host_agents() {
  local reconcileScript="${REPO_DIR}/agents/selfhost/reconcile-host-agents.sh"

  if [[ ! -f "${reconcileScript}" ]]; then
    log "Skipping host-agent reconciliation because ${reconcileScript} was not found"
    return
  fi

  log "Reconciling self-host agents"
  bash "${reconcileScript}" "${REPO_DIR}"
}

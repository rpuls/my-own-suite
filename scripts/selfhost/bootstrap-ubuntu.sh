...
# This file lets USB installer values override the generic VPS defaults.
...
EOF
  ...
  if [[ -n "${MOS_OWNER_NAME}" ]]; then
    set_env_value "${suiteManagerSelfhostEnv}" "OWNER_NAME" "${MOS_OWNER_NAME}"
  fi

  if [[ -n "${MOS_OWNER_EMAIL}" ]]; then
    set_env_value "${suiteManagerSelfhostEnv}" "OWNER_EMAIL" "${MOS_OWNER_EMAIL}"
  fi

  if [[ -n "${MOS_OWNER_PASSWORD}" ]]; then
    set_env_value "${suiteManagerSelfhostEnv}" "OWNER_PASSWORD" "${MOS_OWNER_PASSWORD}"
  fi

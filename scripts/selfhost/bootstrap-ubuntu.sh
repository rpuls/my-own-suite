...
write_suite_manager_selfhost_env() {
  local suiteManagerSelfhostEnv="${REPO_DIR}/deploy/vps/services/suite-manager/.env.selfhost"

  if [[ ! -f "${REPO_DIR}/package.json" ]]; then
    return
  fi

  log "Writing Suite Manager self-host env override"
  mkdir -p "$(dirname "${suiteManagerSelfhostEnv}")"
  cat > "${suiteManagerSelfhostEnv}" <<EOF
# Generated by self-host bootstrap.
...
EOF
  ...
  chmod 0600 "${suiteManagerSelfhostEnv}"
}

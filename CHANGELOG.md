...
- Retired the repo roadmap document in favor of GitHub Issues for roadmap-like task state, tightened the durable decision/agent docs around Homepage-driven proxy annotations, host-agent boundaries, temporary branch planning, local HTTPS/DNS-01 architecture, explicit Homepage URL ownership, and user-run E2E validation, and added Suite Manager validation plus capability-gated Customize UI preview for external-service `mos.proxy` Caddy annotations in Homepage tiles.

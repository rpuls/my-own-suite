...
- Documented the deployment-path convergence direction: Platform deployments remain secondary and currently Railway-focused, while cloud-machine and own-hardware installs should share one Ubuntu 24.04 self-hosted own-infra runtime with USB and cloud-init style installers instead of separate VPS and self-host systems; added early cloud-server installers through `npm run selfhost:cloud-init` provider user-data generation and a fresh-server SSH bootstrap script, routed USB/cloud-init/SSH installs through one shared self-host installer core after `/etc/mos-selfhost.env` exists, captured the alpha direction and progress tracker for control-plane-first installs with Suite Manager app catalog provisioning, added the first current-app inventory, catalog manifests, validation, installed-app state contract, authenticated app catalog API, reusable catalog picker in the Customize add flow, a Stirling PDF install action that applies selected services through the self-host agent, records final installed/succeeded state after host apply, reconciles generated Compose selection from Suite Manager state on catalog reads, derives built-in Caddy app routes from installed catalog selection, refreshes and reloads Caddy routes during host-agent catalog apply, moved seeded Homepage defaults and `vps:up` toward control-plane-first installs, introduced package-directory app catalog ownership for converted apps and package-owned env/Caddy/Homepage/setup-helper projections starting with Stirling PDF and Radicale, moved Radicale setup-helper UI and credential handoff under the Radicale package, made converted app installability/setup-helper/doctor behavior package-driven instead of app-name branches, recreates env projection target services such as Homepage as refresh-only services during catalog apply so installed app tiles are not pruned and Suite Manager is not restarted mid-request, added hygiene tests that keep converted apps out of suite-level defaults and registries, and removed the abandoned `selfhost:cloudflared` tunnel generator from the supported script surface.
- Added a reusable DigitalOcean smoke-test harness with `npm run smoke:do:up`, `npm run smoke:do:reset`, and `npm run smoke:do:destroy` for creating a tagged Ubuntu 24.04 Droplet, running or rerunning the cloud self-host installer from a selected repo ref, saving local logs/state, fast-resetting MOS runtime state on an existing Droplet, and safely cleaning up only MOS smoke resources.
...
- Added app-catalog Compose selection staging through the self-host service agent and `deploy/vps/generated/app-catalog/`, with `vps:up` consuming selected app profiles when a generated selection exists while preserving the current all-app profile behavior when it does not. Compatibility note: `mos-service-agent` now advertises `app-catalog` capability `compose-selection.apply`.
- Made the first catalog install path operational for Stirling PDF on self-host agents: Suite Manager stages selected Compose inputs, asks the service agent to start selected app services without recreating the control plane, adds the app tile to Homepage, restarts Homepage when available, and marks the app installed only after apply succeeds; cloud SSH installs now default to control-plane-only startup for alpha catalog testing.

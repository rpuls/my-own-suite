...
- Hardened V2 app lifecycle status truthfulness with an authenticated runtime status refresh that asks the narrow app agent to re-check an installed app's health projection, records healthy or failed validation state in SQLite, and prevents stale applied projections from continuing to appear as Running after a detected app crash.

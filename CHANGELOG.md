...
- Added a reusable DigitalOcean smoke-test harness with `npm run smoke:do:up` and `npm run smoke:do:destroy` for creating a tagged Ubuntu 24.04 Droplet, running the cloud self-host installer from a selected repo ref, saving local logs/state, and safely cleaning up only MOS smoke resources.

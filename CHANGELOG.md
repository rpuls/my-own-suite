...
- Started a clean root-level `version-2/` workspace for building the Suite Manager-first launch platform separately from the existing suite code, with product-level folders for Suite Manager, future app packages, site, scripts, system agents, shared infrastructure, and canonical V2 branding sync plus a multi-phase roadmap, first V2-local owner setup core covering file-backed state, password hashing, sessions, first-run owner creation, React/Vite owner onboarding, existing-owner login/logout, a signed-in control-plane placeholder, focused HTTP/API tests before later app package lifecycle work, and a tested no-preconfig installer foundation for cloud-init, SSH/bootstrap, USB seed, and a one-command DigitalOcean smoke path with V2-local credentials, state, logs, and corrected Caddy repository/bootstrap wiring that installs the V2 control plane from a selected repo/ref without installer-time owner credentials or app-specific inputs.

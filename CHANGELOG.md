...
- Started a clean root-level `version-2/` workspace for building the Suite Manager-first launch platform separately from the existing suite code, with product-level ownership, canonical V2 branding, browser-created owner authentication, a React/Vite control-plane shell, and a tested no-preconfig installer foundation for cloud-init, SSH/bootstrap, USB seed, and DigitalOcean smoke validation. The V2 control plane now starts Caddy and a digest-pinned private Homepage runtime, exposes separate Home and Suite Manager hosts through Suite Manager, protects the branded Homepage dashboard with host-only Suite Manager sessions, and covers the routing boundary with focused HTTP, streaming, WebSocket, and render tests without installer-time owner credentials, optional apps, or app-specific inputs.

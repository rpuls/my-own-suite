...
- Started a clean root-level `version-2/` workspace for building the Suite Manager-first launch platform separately from the existing suite code, with a multi-phase roadmap for rebuilding the control-plane install, browser-based owner creation, DigitalOcean validation, and later app package lifecycle work in a testable order.

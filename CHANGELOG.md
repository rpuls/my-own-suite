...
- Moved Homepage customization to persistent runtime config seeded from repo defaults and added a Suite Manager Customize screen for allow-listed YAML/CSS/JS edits, so installed suites can change dashboard files without dirtying the production source checkout while preserving generated service-tile pruning. Compatibility note: local/VPS installs now use a `homepage_config` volume mounted at `/app/config` in Homepage and `/homepage-config` in Suite Manager, while `services.yaml` remains generated from `services.template.yaml`.

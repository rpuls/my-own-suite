...
- **Settings → Get help with a problem** puts everything a helper needs into one downloadable file, so an owner who cannot explain what is wrong can just send it: what is running, what failed recently and why, and the relevant service and app logs, led by a summary of what looks wrong. Passwords and app secrets are removed by exact value before it is written and the file reports how many it masked, but **server and app names and local network addresses are kept** — not secret, and a broken address cannot be diagnosed without them — which the file states in its own header. Collection uses a new read-only host agent that a managed update installs for you; the export needs a signed-in owner, so it cannot help someone locked out of Suite Manager.

...
- Removed the leftover v1 owner fields (`OWNER_NAME`, `OWNER_EMAIL`, `OWNER_PASSWORD`) and other unused keys from the self-host USB installer config template and dropped the dead build-time validation that still demanded them; the USB installer now only asks for the machine's Linux login password plus optional domain/timezone, because the owner account is created in the browser after first boot. The own-hardware install guide was updated to match.

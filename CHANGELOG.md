...
- Started a clean root-level `version-2/` workspace for building the Suite Manager-first launch platform separately from the existing suite code, with product-level ownership, canonical V2 branding, browser-created owner authentication, a React/Vite control-plane shell, and a tested no-preconfig installer foundation for cloud-init, SSH/bootstrap, USB seed, and DigitalOcean smoke validation. The V2 control plane now starts Caddy and a digest-pinned private Homepage runtime on one Home origin, serves Suite Manager at `/suite-manager/` with one shared host-only session, seeds the proven MOS Homepage styling, Startpage search, V1 bookmark layout, local MOS/Funkyton tile images, and an editor-ready template into durable state, and covers the routing boundary with focused HTTP, streaming, WebSocket, branding, and render tests without installer-time owner credentials, optional apps, or app-specific inputs. Compatibility note: the pre-release V2 `suite-manager.<domain>` host and `/setup/` Home path are replaced by `home.<domain>/suite-manager/`.

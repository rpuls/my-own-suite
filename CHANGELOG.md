...
- Added a new standalone `mail-relay` service under `apps/` as a transparent SMTP-to-SMTP bridge for low-volume transactional mail, with sender rewrite and lightweight rate limits, so the relay stays provider-agnostic.

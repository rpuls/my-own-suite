...
- Started a clean root-level `version-2/` workspace for building the Suite Manager-first launch platform separately from the existing suite code, beginning with control-plane install, browser-based owner creation, and DigitalOcean validation before migrating any optional app catalog behavior.

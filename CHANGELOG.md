...
- Started a clean root-level `version-2/` workspace for building the Suite Manager-first launch platform separately from the existing suite code, with product-level folders for Suite Manager, future app packages, site, scripts, system agents, and shared infrastructure plus a multi-phase roadmap and first V2-local owner setup core covering file-backed state, password hashing, sessions, first-run owner creation, and minimal HTTP/API tests before later app package lifecycle work.

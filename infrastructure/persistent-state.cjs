...
// at that relative path and restored back to `path`; entries with
// `emptyContent` are reset to it before a restore rebuilds them. The backup
// engine stages and resets exactly this list, so adding a managed location
// means adding an entry here, not editing engine code. Volumes are enumerated
// dynamically via `classifyVolumes` and appear here as the contract entry
// `app-volumes`.
function managedStateTargets({ caddyDir = '/etc/caddy', secretsDir = '/etc/mos/secrets', stateDir, stateRoot }) {
  return [
    ...
    // The address this suite publishes on. Never backed up and never restored:
    // it belongs to the machine, and a restore that carried it would move the
    // suite off the address its owner is standing on.
    {
      backedUp: false,
      class: 'machine-local',
      id: 'suite-address',
      kind: 'directory',
      path: `${stateRoot}/suite-address`,
    },
    // Written by the installer and rewritten by the domain module; it survives
    // a restore untouched because it belongs to this machine's address.
    {
      backedUp: false,
      class: 'machine-local',
      id: 'caddy-Caddyfile',
      kind: 'file',
      path: `${caddyDir}/Caddyfile`,
    },
    // Projections of the restored database and Homepage config. A restore resets
    // each to its empty content and rebuilds it, so nothing this machine served
    // before the restore can survive beside what the backup names.
    {
      backedUp: false,
      class: 'generated-runtime',
      emptyContent: '# No user-managed Homepage routes.\n',
      id: 'caddy-mos-homepage-routes.caddy',
      kind: 'file',
      path: `${caddyDir}/mos-homepage-routes.caddy`,
    },
    {
      backedUp: false,
      class: 'generated-runtime',
      emptyContent: '# No app runtime routes.\n',
      id: 'caddy-mos-app-routes.caddy',
      kind: 'file',
      path: `${caddyDir}/mos-app-routes.caddy`,
    },
    {
      backedUp: true,
      class: 'authoritative-persistent',
      ...
      kind: 'file',
      // Restored to `parkedPath`, never to `path`: the live file is read by
      // Caddy at process start, so a restore that wrote it would leave the
      // running Caddy holding a token it cannot see — which is exactly how a
      // successful restore once came back serving nothing. Only the domain
      // module writes the live file, and it restarts Caddy when it does.
      parkedPath: `${secretsDir}/caddy-cloudflare.env.parked`,

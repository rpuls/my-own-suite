...
- Every site block that proxies Suite Manager carries a `handle_errors` handler serving `/etc/caddy/mos-status/unavailable.html` as `503` with `Retry-After: 15`, so the deliberate control-plane outage in the middle of a restore answers with a self-refreshing page instead of a bare `502`. It catches only errors Caddy raises itself, so a genuine 404 or 500 from Suite Manager still reaches the browser. The installer writes the page on a fresh machine and `reconcile-system.cjs` rewrites it on every run, unlike the Caddyfile it sits beside; because that file is written once and is replaced wholesale when HTTPS is applied, an already-installed Caddyfile is upgraded in place instead — the handler is added to every top-level block that proxies Suite Manager and has none, and the edit is kept only if `caddy adapt` accepts it.

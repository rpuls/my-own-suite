...
- Reuse existing shared components before creating new local controls. Current shared primitives live in `suite-manager/frontend/src/components/` and include dialog frames, drawers, action menus, notices/alerts, text inputs, text areas, selects, confirmation checkboxes, setting switches, and the advanced panel.
- Checkbox and switch are two controls, not one with two skins. A checkbox is a deliberate acknowledgement that gates something else and is submitted with it; a switch is a setting that takes effect the moment it is flipped. Pick the one that matches, and do not add a third control the next time neither quite fits — extend one of these.
- A control's own look — track, knob, gradient, focus ring — is shared MOS branding and belongs in `branding/styles/mos.css` under a `mos-` class, so it stays identical wherever it is used and matches the design-system project. The product stylesheet owns only where that control sits: row layout, spacing, and surrounding copy.
...
- **A panel that holds a list, or more than one region, uses the flush panel sections.** `Panel` with `PanelHead`, `PanelBand`, `PanelList`/`PanelItem` and `PanelBody` (`.mos-panel-flush` and friends in `branding/styles/mos.css`) make one surface whose sections run edge to edge and are told apart by hairlines. The inset belongs to the section, never to the panel: that is what lets a row tint its full width, and what stops a bordered list inside a padded panel inside a page surface from reading as an onion. Do not re-create the shape with a local bordered list. `--mos-panel-pad` is a root token, so the same sections work inside a dialog, a drawer, or a page on the public site.
- A flush panel clips to its own radius, so anything that pops out of a section — a menu, a popover, a tooltip — must portal to `<body>` like the shared `Dialog`, `Drawer` and `ActionMenu` already do. That is the same requirement THE FROST RULE already imposes.
- Chips and pills must stay legible on any surface, including on top of an app screenshot. Legibility comes from the primitive's own `backdrop-filter` blur plus its border and label, not from an opaque plate behind it: do not give a chip a solid fill, a sheet-coloured wash, or panel glass to make it readable over an image. The exception is a control floating on the unfaded part of a cover image, where no theme colour is safe and the chip carries its own dark glass in both themes.
- Use the shared `AdvancedPanel` for low-level technical information that is useful for support or debugging but intimidating for normal users. Preserve the details, but keep raw logs, generated config, command output, IDs, paths, and similar diagnostics inside the panel while the primary UI explains the user-facing state in plain language. Never hand-roll a disclosure with the `suite-advanced` class; a unit test fails if that class appears outside the shared component and the stylesheet.
- `AdvancedPanel` takes a required `reveal`. Use `reveal="on-failure"` only where the surrounding UI is already reporting that something went wrong, because that panel renders for every owner. Everything else is `reveal="technical-mode"`, which renders nothing at all until the owner turns on **Settings → Technical controls**; `reveal` may be computed when one panel serves both contexts.
- What may go behind the gated mode is bounded: facts MOS already generated, and overrides the owner genuinely owns. It is never where a feature lands because its UI was not finished, and never a home for a control that ordinary owners need. Gating an unfinished feature turns the mode into a junk drawer and puts the burden on exactly the owners least able to carry it.
- Reach for `useTechnicalControls()` only when an entire section rather than a disclosure is technical-only, and say in a comment why the panel was not enough. The panel gating itself is the point: a screen that writes its own `enabled ? ... : null` can leak a new advanced surface by forgetting to.

...
- **C6 — Record the own-hardware install walkthrough.** The second video, and the one **C7** exists to
  make filmable: download the published ISO → Rufus or Etcher → boot → save the server login → owner
  account. Blocked on **C7**, because filming today would mean filming Node and Docker being installed
  on the viewer's laptop — a flow that is about to stop being the real one. *(Owner-run — after C7)*
...
**Gate:** every official release publishes a verifiable installer image, downloading and flashing it is
the whole own-hardware path, and nothing about the machine's identity or credentials comes from the
image rather than from the machine.

Promoted from **L7** and from the standing "own hardware is for technical users" position, which this
theme retires. The reasoning is that building the ISO once is strictly better than ten thousand people
each installing Node and Docker to build the same bytes — it removes the two prerequisites that make
the own-hardware path technical at all, and it is the difference between a filmable walkthrough and an
unfilmable one (**C6**).

- **H1 — Build and publish the installer image from the release pipeline.** `npm run installer:usb`
  already produces the ISO; the work is running it in CI per tagged release, pinning `MOS_REPO_REF` to
  that tag instead of the builder's current branch, and publishing checksums. **Hosting is not GitHub:**
  the Ubuntu 24.04 base is 3.17 GiB and a release asset is capped at 2 GiB. Cloudflare R2 is the fit —
  zero egress, an account already exists for Pages, and lifecycle rules can retire old images while the
  release notes keep the checksum. *(Medium)*
- **H2 — Nothing machine-specific may originate in the image.** One build is flashed by everyone, so
  anything decided at build time is shared by every install and extractable from the download. The
  console password now generates on first boot; SSH host keys are generated by the installer on the
  target and need confirming rather than assuming, and any future baked value needs the same test.
  Absorbs the old **AL5**, whose remaining question — does subiquity accept a locked `identity.password`
  — is a gate on this theme. *(Small)*
- **H3 — Boot-test the published image before the release publishes.** Boot the built ISO in QEMU in CI
  and assert MOS comes up. Without it, the first person to discover a bad image is a downloader.
  *(Medium)*
- **H4 — Decide what an unattended installer is allowed to do to a stranger's disk.** The seed installs
  ...
  driver. Needs a visible target-disk confirmation and a decided answer for multi-disk machines before
  the image is public. *(Small — needs a decision first)*

Also settle before publishing: the image is a stock Ubuntu base plus an autoinstall seed, so name and
describe it so it does not read as a Canonical product; and **H1** makes **E3** load-bearing rather
than aspirational, because a flashable image that installs an OS and runs a root shell script is the
highest-trust artifact the project ships.
...
- **AL1 — Host OS patching.** MOS updates itself and it updates the apps, and nothing updates Ubuntu;
  ...
  state where owners already look for updates. **H1 raises this** from small to load-bearing: a
  published image is frozen at release time, so someone flashing a months-old download starts behind on
  day one. *(Small)*
- **AL2 — Full-disk encryption for own-hardware installs.** The installer uses a plain disk layout, so
  ...
  TPM-backed, or network-bound — before it is buildable. **H2 narrows the options:** a published image
...
- **AL5** — moved to **H2**. The placeholder hash is gone and the password is generated on the machine;
  what remains is confirming a locked `identity.password` on a real boot, which is a gate on the
  published-image theme rather than an independent item.
...
- **AL7 — Recovery onto a new machine at the version the bundle expects.** The backup guide tells an
  ...
  and accepting it. **H1 supplies part of the answer for own hardware:** a per-release image is an
  installer pinned to a known version, which is exactly what the bundle's instruction asks for.
...
- **E3 — Signed release and installer artifacts.** The catalog and advisories are Ed25519-signed; the
  release and the pipe-to-shell installer that delivers everything are not. **H1 makes this a gate
  rather than a nice-to-have** — a downloadable image that installs an OS and runs a root shell script
  is the highest-trust artifact MOS ships, and it will be served from object storage rather than from
  the repository people trust. Published checksums plus build provenance from the release pipeline.
  *(Medium)*

...
- [x] Extended the Radicale package slice with manifest-driven Homepage widget projection: package manifests can carry a constrained calendar widget and structured tokenized internal iCal bridge, Suite Manager keeps widget/bridge secrets as placeholders in public projections, the app agent renders the Caddy bridge from structured fields, and the Homepage add-link path materializes only the tokenized widget URL.
- [x] Added Seafile core as the first serious V2 multi-service app package: package-owned Seafile, MySQL, and Valkey Dockerfiles; one public `seafile.<base-domain>` route; internal-only database/cache services on a package network; generated database/JWT secrets; user-supplied initial Seafile admin credentials; `/shared` and MySQL persistence; structured onboarding notes; and tests for manifest validity, multi-service projections, secret redaction, app-agent multi-service apply/remove, and no production core hardcoding.

### Current Next Slice

Build the next real V2 vertical slice inside `version-2/`:

1. Validate Seafile core in the Hyper-V USB smoke path before treating multi-service app packages as operationally proven.
2. Continue hardening V2 secret management before expanding the app package surface further. The current restricted-file secret storage now has a controlled materialization failure boundary and Hyper-V validation, but it is still an early recoverable-secret mechanism rather than the final secret management system.
3. Continue hardening the app lifecycle path around Stirling PDF, Vaultwarden, Radicale, and Seafile, especially failure recovery, richer user-facing setup/onboarding rendering, show-once or user-managed secret reveal/rotation policy, and eventual reinstall/cleanup decisions for apps that were uninstalled with data preserved.
...
- Radicale validates the one-service package path for V1-era sync apps: a package-owned entrypoint can perform app-local bootstrap work such as htpasswd setup and default collection creation while the app agent still only sees declarative env, volume, route, internal bridge, and health projections.
- Homepage widgets should be package metadata, not Suite Manager special cases. The first supported widget shape is a constrained calendar widget plus a structured tokenized iCal bridge; public projections keep `${secret...}` placeholders, runtime apply materializes bridge credentials, and Homepage apply materializes only the bearer-style widget URL.
- The V1 Vaultwarden handoff, DAV auto-discovery, richer widget types, and multi-user/device onboarding are intentionally deferred. Adding them should happen as explicit package-contract work rather than by hardcoding Radicale into control-plane core.

First multi-service package findings from Seafile:

- The manifest service map was already close to multi-service shape, but runtime apply still had one-service assumptions. The app agent now accepts a bounded multi-service projection, creates a package-owned Docker network for companion services, publishes only services referenced by structured routes, and removes all declared package containers during disable or preserved-data uninstall without deleting volumes.
- Per-service loopback ports must be deterministic by package and service id, not only package id. Caddy routes target only routed services, while health binds to the service named by the health URL host.
- Seafile core should collect the initial admin email/password from the owner because the current V2 secret mechanism stores secret references without an owner-facing reveal flow. Generated secrets are still appropriate for MySQL root/user passwords and the Seafile JWT private key.
- OnlyOffice remains a separate integration slice. The V2 Seafile package carries only proxy-facing Seahub patching from V1; V1's OnlyOffice and SMTP runtime patching are intentionally left out until the package contract has explicit integration semantics.

...
- [x] Added Seafile core as the first serious V2 multi-service app package: package-owned Seafile, MySQL, and Valkey Dockerfiles; one public `seafile.<base-domain>` route; internal-only database/cache services on a package network; generated database/JWT secrets; user-supplied initial Seafile admin credentials; `/shared` and MySQL persistence; and tests for manifest validity, multi-service projections, secret redaction, app-agent multi-service apply/remove, and no production core hardcoding.

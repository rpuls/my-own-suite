...
- [ ] Display requested permissions before install and permission increases before update. (`describeRequestedPermissions`/`diffRequestedPermissions` primitives exist and the `POST /apps/sources/:id/preview` route returns the requested-permission set and trust for owner consent. The before-install half now ships in the UI: the external package detail view lists each requested route/volume/integration in plain language before any install. Permission-increase-before-update display still depends on the external update flow.)
...
- [ ] A valid unverified example installs through the same snapshot/app-agent pipeline with visible unverified status. (`ExternalSourceClient` produces a validated unverified candidate through the shared package pipeline, and the Apps UI now surfaces its visible unverified status in the resolve preview. Snapshot persistence and app-agent apply for external installs remain pending; the UI Install action is disabled-and-labeled until that external install path lands.)

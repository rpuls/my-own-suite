...
- [ ] Display requested permissions before install and permission increases before update. (`describeRequestedPermissions`/`diffRequestedPermissions` primitives exist and the `POST /apps/sources/:id/preview` route returns the requested-permission set and trust for owner consent. The before-install half ships: the external package detail view lists each requested route/volume/integration in plain language, and it is now shown before a real install rather than a disabled one. Permission-increase-before-update display still depends on the external update flow, which does not exist yet.)

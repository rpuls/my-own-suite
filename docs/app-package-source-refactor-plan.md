...
- [ ] Display requested permissions before install and permission increases before update. (`describeRequestedPermissions`/`diffRequestedPermissions` primitives exist and the `POST /apps/sources/:id/preview` route now returns the requested-permission set and trust for owner consent; UI display is pending.)

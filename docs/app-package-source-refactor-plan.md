...
- [x] Choose and document catalog/package signing and key distribution/rotation. (Ed25519, one publisher key, distributed by shipping its public half in the release at `trust/official-catalog.pub` and read from the installed checkout — never fetched, because a key served by whoever served the catalog only proves they agree with themselves. Chosen over sigstore/OIDC keyless because a self-hosted box must be able to verify its cache with no network, no clock, and no dependency on a transparency log staying reachable; the cost is a private key the publisher has to hold, kept out of the repo and read from `MOS_CATALOG_SIGNING_KEY` at signing time. Rotation is a MOS release carrying the new public key: an installed box only accepts a new key by taking a release, so a compromised repository cannot rotate the key out from under it. Signatures are detached (`apps/catalog.json.sig`, `apps/advisories.json.sig`) and taken over LF-canonicalized bytes, because `* text=auto` means a Windows working tree hands the signer CRLF while raw.githubusercontent.com serves the LF the commit holds — signing the file as it sits on disk verifies only on the machine that produced it.)
- [x] Verify signatures before granting `mos-reviewed` or `publisher-signed` trust. (The catalog decides which packages MOS treats as reviewed and which installed versions have advisories against them, and the only thing behind that was "GitHub served it from the commit we asked for" — so anyone who could push to the repository, or convince a box it was talking to GitHub, could change that decision everywhere at once. `OfficialCatalogService` now verifies the catalog against the release's key before parsing it, and a release with no key refuses to construct at all rather than trusting whatever arrives. The cache is verified on the way in as well as out: it decides trust for as long as a box stays offline and it lives in state rather than in the release, so it stores the signed bytes and the signature rather than the parsed result — a parsed catalog cannot be re-verified because re-serializing it does not reproduce what was signed — and a cache that no longer verifies is discarded rather than served. Cache schema is 2; a version-1 cache is simply refetched. An advisory feed that does not verify is dropped without failing the catalog refresh, because a feed is only ever a reason to warn; both failures are counted, and `app-catalog-signature-invalid` is kept apart from `app-catalog-refresh-failed` so "the network is down" cannot bury "something served this box a catalog nobody signed". `readSigningPublicKey` refuses a PEM containing a private key: Node derives a public key from a private one without complaint, so a release that shipped the signing key in the public key's place would verify every catalog perfectly and give no sign the secret had been published. `npm run release:check` and `apps:catalog:check` both verify the committed signatures, because a stale one fails on every installed box rather than on the machine that cut the release. `publisher-signed` is now refused outright at `buildSourceRecord`: there is no publisher key to check a signature against, so any string bought the "signed by the publisher" label an owner reads before deciding to install, and a label nothing stands behind is worse than the unverified one it replaced. The signature is still stored, unpinned and buying nothing, as what a later publisher-key check would compare against. Not covered: publisher key registration/pinning for external sources, so every external source is unverified and Phase 8's `publisher-signed` trust level is currently unreachable by design; signatures prove who wrote what a box was given and never that it was given everything, so a withheld advisory feed still reads as "nothing to warn about", which needs freshness/expiry rather than signing; and the local-checkout install path is still mos-reviewed by virtue of being part of the installed release, which is the same trust the release's own code already has.)
...
- [x] Add bounded storage cleanup for candidates, previous snapshots, and unused images. (Candidates: `candidate-storage.cjs` owns `stateDir/app-candidates`, the one place an unvalidated package may land. A killed Suite Manager used to leak its download there forever with no bound at all; the root is now swept before every download and once at startup, stale-first then oldest-first, and never touches a directory an in-flight operation in this process owns. Previous snapshots: `promoteAppPackageUpdate` retains exactly one `previous` per instance. Unused images: every image tag embeds the package digest, so each update used to leave its predecessor behind *tagged* — not dangling, so `docker image prune` never touched it — and growing by one generation per update forever. Promotion now reclaims them, under app-agent contract 8 and the `apps.package.update.reclaim` capability. The agent names the outgoing images from the manifest whose digest it just verified, so a caller cannot widen a reclamation by describing the old package as something it is not; the only new input is the outgoing source revision, which is not recoverable from a snapshot because it is deliberately not part of the digested package. A rollback-safe update keeps its predecessor's images and records their tags next to the retained snapshot, so the next promotion reclaims the generation it evicts rather than accumulating one per update; a rolled-back update reclaims the abandoned candidate's images, which a retry rebuilds from the same cached layers. Removal is `docker image rm` without `--force` with every failure ignored, so an image a container still references stays and reclamation can never break a running app or undo a committed update. `installedSourceRevision` is an optional promotion field gated on the capability: an older agent rejects unknown fields outright, and refusing a promotion after the candidate is already serving traffic would strand a committed update, so a version mismatch leaks an image instead. Uninstall closes the other end, under contract 9 and `apps.package.remove.reclaim`: it used to leave the app's final images *and* its whole instance directory behind, and because the instance row is deleted on uninstall, that directory was unreachable for the rest of the host's life. `remove` now optionally takes the instance and the installed revision, reclaims the final images plus any generation retained for rollback, and removes the instance directory; the tags are named from the snapshot's own manifest, digested on disk, so a snapshot that is not the package being uninstalled names nothing. Both fields are gated the same way as the promotion field, and are rejected on `stop`, which must stay reversible.)
- [x] Add catalog metrics/security events without URLs containing credentials or sensitive query data. (Three things worth counting were invisible: a source serving a package the fail-closed gate refused, a source tripping its own download bound, and an official catalog that cannot refresh. The first is the one that matters — a single rejection is an owner pasting the wrong repository, but the same source doing it repeatedly is the shape of a repository that has been taken over or force-pushed, and until now the only trace was an error shown to whoever happened to click. A failing catalog refresh is worth counting because it is quiet: the last-known-good cache keeps serving and nothing looks wrong, while MOS has stopped learning which installed packages have advisories against them. The URL requirement is met structurally rather than by scrubbing: the recorded subject is always an opaque id — a client fingerprint, the source id, a digest of the catalog repository — so no URL, credential, or query string is ever in hand to leak, and a test asserts the recorded events match no URL-shaped text. `security_events` was shaped around the only event that existed, a throttled sign-in, which always has a client and always has a retry delay; migration 12 rebuilds it (SQLite cannot widen a `CHECK` in place) with a generic `subject`, an optional retry delay, and the widened type enum, carrying existing rows across. The summary is broken out per type, because a total mixing throttled sign-ins with refused packages reports a number that means nothing. Recording never participates in the failure it observes: every write is wrapped, so a store that cannot take an event leaves the rejection, throttle, or refresh failure reporting exactly as it did. `APP_DOWNLOAD_BUSY` is deliberately not counted, because that bound is host-wide and says only that MOS was busy with other sources. Not covered: there is still no read surface — `getSecurityEventSummary` has no route and no UI, as was already true of the sign-in throttling this table was built for, so today these are a durable record rather than something an owner is shown; and the wiring from `createHttpApp` into the catalog service and external client is constructor-level only, exercised in production but not by a test, because the HTTP tests inject their own services.)
...
- [x] Add multi-architecture artifact validation. (Base images are pinned by digest, and a digest resolves to one architecture's image or to none, so a package pinned to amd64 manifests could only fail on an arm64 host in the middle of `docker build` — after the download, the gate, and the snapshot had all passed — with a raw docker error. Nothing declared, reported, or checked an architecture anywhere: the only record that Immich is amd64-only was a sentence in its README. A manifest may now declare `architectures`, the app agent reports the host it builds on (it runs `docker build`, so it is the authority; Suite Manager may be a container built for something else), and official install, external install, update preview, and update staging all refuse a package the host is not named in. Immich declares `amd64` at package version 0.3.0, which is the version bump the digest change requires rather than an integrity error on existing installs. Deliberately not a trust control and deliberately fail-open on every unknown: the declaration is the publisher's word, not registry introspection, and nothing here can inspect a registry to check it; a package that declares nothing, a host the agent does not recognise, and an agent too old or unreachable to answer all mean "no constraint", because this check exists to explain a build failure that was already coming and must never invent one. A declaration MOS cannot parse is refused rather than skipped, since ignoring it would silently drop the constraint the package meant to state. Not covered: the generated catalog does not carry `architectures`, so an incompatible app is still listed as installable and refuses at Install rather than being marked in the list; and `docker build` still passes no `--platform`.)

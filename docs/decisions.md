...
- **One destination answers for every automatic trigger.** `primary-destination.json` holds it, `POST /v1/destinations/primary` sets it, and a schedule cannot be turned on before one exists. It is chosen next to the schedule, not on a destination card, where it would read as a property of the drive; the card only says **Automatic backups** so the list answers the question at a glance. A schedule set up before this release loses its destination and must be pointed again; nothing migrates.

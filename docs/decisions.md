...
- App-agent contract version 5 adds digest-bound snapshot promotion after candidate activation. Suite Manager now advances an update through candidate build, candidate health, cross-app network reconciliation, agent-owned snapshot promotion, and one SQLite identity/projection commit. Promotion uses same-filesystem renames, re-verifies both package digests, and retains exactly one prior snapshot only when the candidate declares rollback `safe`; otherwise the displaced definition is removed after promotion. Deterministic named volumes remain attached across the runtime replacement. Failures retain the latest durable operation stage, including the explicit case where the candidate is healthy but integration reconciliation or post-promotion database recovery is still required. Automatic startup recovery and Homepage projection reconciliation remain required before Phase 5 can be considered fully transactional.

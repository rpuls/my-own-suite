...
- External updates are not implemented. The namespaced id and `renderDryRunProjections` already take the installed package id, so the update path can reuse it, but permission-increase-before-update display is still outstanding.

...
Decision: The address a suite publishes is a **recorded fact with one owner**, not a value each module derives for itself. It lives in one machine-local file, `/var/lib/mos/suite-address/address.json`, as `{ kind, host, scheme }` with `kind` one of `lan-name`, `easy-door` or `domain` (a `domain` also carries `baseDomain`, `acmeEmail` and the provider, for the domain module's own use). Everything that builds a URL — Caddy's route fragments, app `${app.publicUrl}`, Homepage hrefs, Suite Manager's own links — reads that file and nothing else. Only Suite Manager writes it, at three moments: the first time it starts on a machine with no file it records the install-time name; onboarding records the `Host` and scheme the owner completed setup on, which is the moment the owner *picks a door*; and the change-of-address transaction (`SuiteAddressService.change`) writes the new address. That transaction, for a domain, has the HTTPS agent rewrite the web server, restart it and **wait for a trusted certificate for the name** inside the agent's own rollback; records the address; commits; and only then re-bakes every app and re-renders Homepage, reporting the apps that did not follow rather than failing a move the owner may already be looking at through the new name. The HTTPS apply is one caller of that transaction, not a separate path; following a moved Easy Door name is another. A **door is not an address**: the Easy Door and the install-time name are ways in that Caddy and Suite Manager's host gate answer on for Suite Manager, always, whatever the recorded address is; only apps and Homepage are single-addressed. The file is `machine-local` in `managedStateTargets`, so a restore never reaches it — the restore module restores settings and data and cannot change where the suite is, because it cannot see the thing that says where the suite is. A backup's manifest carries the domain its machine was on and the contact that issued its certificate; the restore writes them to `offered.json` beside the address — its own file, so the restore never opens the address file at all — and parks the credential, and Settings turns the offer into the same transaction, with the parked credential, when the owner asks.

Reason: The previous shape of this entry — written hours earlier, on the same branch — said the address was "answered once, in `shared/served-address.cjs`, in one order: a domain that is being served, then the Easy Door, then the install-time name". A review the same day found that module had one caller. Homepage derived the address as `settings.baseDomain || bootstrap` with no Easy Door at all; Settings showed `http://home.mos.home/` as the machine's active address; `appPublicUrlFor` used the request's Host. Three derivations, three answers, and no place anywhere that recorded which door the owner had actually come through — so "the suite adapts to the door you picked" was not implementable, only approximated. Deriving the address is the fault. Every derivation is a guess from raw inputs (a settings row, an environment variable, the live network interface, a request header), and the inputs disagree in exactly the situations that matter: a restored machine whose install-time name points at the server being replaced, an owner who came in through the Easy Door, a domain applied on a name the owner's DNS never learned. Recording it once, at the moment a request reveals it, is the only way a background job with no browser in hand — a restore, a reconcile, a scheduled backup — can know the answer at all.

The second fault was treating a door and an address as one thing. That is why applying a domain removed the Easy Door: closing a door and changing an address were the same operation. It is also why an owner who configured HTTPS while browsing through the Easy Door could be stranded on success — the apply deleted the origin their screen was retrying against. Suite Manager answering on a name is harmless and is the recovery path; *publishing* links on a name is what has to be single-valued. Once those are two concepts the Easy Door never needs closing, and the posture argument from 2026-08-16 — plain HTTP beside HTTPS — is answered by keeping the door for one page rather than for every app.

The third fault was that changing the address was a settings write with best-effort follow-ups. The HTTPS apply reported success when Caddy restarted, then reconciled app URLs in a `try` whose failure became a field in the response. Apps bake their address into their own configuration — Vaultwarden's `DOMAIN`, Paperless's `PAPERLESS_URL`, single-valued and locked at manifest generation 1 — so a change of address is a rebuild of every app that can partly fail, and partial application was the normal shape, not the exception.

Consequences:

- **A restore has no address code.** No capture, no settle, no park in the settings row. The file is not in the backup, the restore loop does not touch machine-local targets, and the suite comes back on the recorded address by construction. The nine combinations of "machine on X, backup from Y" collapse to one path with no matrix.
- **The database holds change bookkeeping only.** `https_settings` is dropped; `address_changes` holds the one change that can run at a time — its stage, its target and how it ended — and never a credential or the address. What is served is in the address file; what is offered is in the offer file beside it. The column that carried both an in-flight apply and a set-aside domain, and was nulled by a failed apply, goes.
- **The web server always answers on every door for Suite Manager.** `renderHttpsCaddyfile()` keeps the Easy Door and install-time blocks; the host gate admits the recorded host, the live Easy Door name and the install-time name, always. Apps alias the Easy Door while `kind` is not `domain`, decided from the file rather than from a marker in the live Caddyfile. The "closed by a real domain" consequence of 2026-08-16 is superseded; its reason for apps staying single-addressed is kept.
- **A change of address is fire-then-poll on the screen.** Caddy restarts under the owner's open connection, so the reply to the request that started the change is lost by design. The Settings screen uses the same poll classification the Backups screen gained on 2026-09-20 — signed out, refused at this address, unreachable, answered — and never renders a state it has no reason to believe. The apply used to paper over this with eight same-origin retries.
- **Success means the name is served with a trusted certificate.** For `kind: domain` the HTTPS agent's apply completes a verified TLS handshake for `home.<domain>` against the local web server before it answers, and rolls the web server back if none arrives in time; the change reports each stage. Whether the name *resolves* to this machine is reported on the status, never enforced — the owner creates the DNS override after the certificate exists, and MOS reads DNS without writing it. "Caddy restarted" is no longer "applied".
- **Address drift is observable.** When the recorded kind is `easy-door` and the live Easy Door name differs, the machine's address moved under it (a DHCP change); status says so and offers the same transaction. The client already asks DHCP for its previous address on every renew, so a reservation on the router remains the only prevention and MOS's job is to notice, not to prevent.
- **A machine always has an address file once Suite Manager has started on it.** The installer and `reconcile-system` create the directory, owned by the runtime user so the root agents can read what Suite Manager writes; Suite Manager records the install-time name the first time it starts with no file. Nothing reads the old settings row to seed it: a machine updated from before this entry comes up on its install-time name and its owner applies the domain again from Settings, which is the no-backwards-compatibility rule applied to Rasmus's own server. Every consumer relies on the file existing; an agent that runs before Suite Manager ever has says so rather than guessing.
- **The same transaction serves A9 and A10.** "Use the offered domain here" and "take the old server's address" are both `changeSuiteAddress` with a different `to`.
- **Everything from the earlier shape of this entry that was not about derivation still holds** and is kept below: the irreversible choice left the blind window, recovery never needs a credential inside the thing being recovered, the key follows the data, a provider name is never a predicate outside its module, state is classified by owner, only the owning module writes a file, a refused reload is reported, a restore surface shows only what it can see, and rehearsal is safe.

## 2026-09-20: A Restore Never Changes The Address Its Owner Is Standing On

Decision: A restore decides nothing about addresses, and **never changes the address the machine is on**. The address a machine serves is a property of that machine, not part of a backup's payload — and as the entry above records, it is held in one machine-local file the restore never touches. A domain carried by a backup from another machine is **set aside** as an offer, and the suite is rebuilt on whatever door the owner came in through: the Easy Door, the install-time name, or a domain this machine was already serving. Serving a name is the domain module's job and happens nowhere else; the restore module may ask what address is served, never set one. The owner is told beforehand that the carried name will not follow, and afterwards what is still pointing at the machine they came from. **MOS reads DNS and never writes it.**

Reason: The previous design asked the owner, before a restore, whether to move the address to this machine or restore as a copy, and acted on the answer in the middle of the run — the one stretch where Suite Manager is deliberately stopped and no screen can report anything. On 2026-09-19 a real restore onto new hardware took that branch and succeeded: six apps and fifteen volumes verified, no warnings. It then rewrote the machine's Caddyfile for `home.<domain>` and reloaded. The reload failed, because the running web server had started before the restore put the provider credential in place and an environment file is only read when a process starts — so the machine kept serving its pre-restore routes, while the restored Suite Manager, reading the restored settings, refused the Easy Door address its owner was on. Caddy routed only the name Suite Manager rejected; Suite Manager accepted only names Caddy did not route. A successful restore locked its owner out of their own data with no path back through the product, and the browser showed a progress spinner throughout.

The deeper fault was not the reload. It was that the last act of a blind window changed the address the owner was standing on, so any failure there was unrecoverable by definition. The network dependency makes it worse and cannot be automated away: a home setup typically points a wildcard at one machine's address through a record MOS cannot see or edit, so even a flawless move leaves every device broken until a human changes something outside MOS.

Consequences:

- **The irreversible choice moved out of the blind window.** The pre-restore dialog states what will happen; the decision that remains — serve that name here — is made afterwards, when it is visible, checkable and undoable. This generalises: MOS does not ask, inside a window where it cannot report, a question whose answer can remove the screen that would report it.
- **Recovery never requires a credential stored inside the thing being recovered.** The suite returns on an address the machine already answers on, needing nothing from DNS or a certificate authority, so the owner reaches their password manager within minutes rather than needing the token inside it first. The credential the backup carried is kept aside on the machine for the same reason. Every future recovery path is checked against this: an owner must never need something only the broken system holds.
- **The key follows the data, not the address.** A restore of another machine's backup always takes on that machine's recovery key, because this machine now holds that suite. It used to depend on the address plan, which would have left a machine holding a suite with its own key for the disk and a borrowed one for the archive — a state 2026-09-18 already rejected.
- **A provider name is never a predicate outside the module that owns it.** The restore's park filtered on `tls_mode = 'cloudflare-dns01'`; a second provider would have matched no rows, set nothing aside, and reproduced this lockout exactly. The park is gone with the settings row (see the entry above); the rule stays. Module-specific vocabulary stays inside its module; other modules ask questions, not string comparisons.
- **State is classified by who owns it, and the address is owned by the machine.** `managedStateTargets` already separated `machine-local` from `authoritative-persistent`; the address simply sat on the wrong side of that line. It is now a `machine-local` file (entry above), the `Caddyfile` is `machine-local` beside it, the two route fragments are `generated-runtime` with an `emptyContent` the restore resets them to before rebuilding them, and the provider credential has a `parkedPath` it is restored to instead of its live path. Anything a restore carries but must not install, or must reset before it rebuilds, is expressed as a field on that table, never as a special case in the restore loop.
- **Only the module that owns a file writes that file, including its secrets.** The restore used to write the live provider credential and then ask the web server to reload, which is the precise shape of the lockout: an environment file is read at process start, so the running server could not see what had just been written. It also silently replaced the credential of a machine that already had HTTPS of its own, which kept working until the next restart and then failed to renew — a fault that would have surfaced weeks later with nothing pointing back at the restore. The domain module writes that file and restarts the server when it does.
- **The install-time name is not "where this machine publishes".** It describes where a machine was born, which on restored hardware can name the server being migrated away from — on the machine this entry came from, `home.mos.home` resolved to the old server. It is one door among two; where the suite publishes is the recorded address (entry above), which onboarding sets to the door the owner actually came through.
- **A reload the web server refused is reported, not discarded.** It was an `optionalCommand`, so a machine whose routes never went live reported success. It does not fail the restore — the data is back either way — but it is recorded on the job and said in plain words, because the silent version is an unreachable suite claiming to be fine.
- **A restore surface shows only what it can currently see.** Polls no longer fail silently: the screen distinguishes signed out, refused-at-this-address, unreachable and answered, reads the progress file the web server publishes while Suite Manager is down, and says how long it has had no answer. The incident's screenshot showed step 1 of 9 while the machine was at step 7, nine minutes earlier — a page cannot be allowed to render a reading it has no reason to believe.
- **Rehearsal became safe, which is the point.** Restoring onto a spare machine no longer touches the live domain, so "try a restore onto another machine some weekend" is advice MOS can now give. Nobody practises this drill, and the old design made practising it dangerous.

## 2026-09-18: The System Disk Stays Plaintext, The Vault Opens Itself, And An Owner Can Make It Ask For Their Password

Decision: Own-hardware installs divide the disk in two. The system partition grows to a cap — 15% of the disk, floored at 12 GiB and capped at 24 GiB — and stays unencrypted, because it carries only Ubuntu, a kernel and the MOS checkout: public, identical on every install, downloadable from us. Everything past it is one LUKS2 partition, the vault, holding `/var/lib/docker`, `/var/lib/mos` and `/etc/mos/secrets`, each bind-mounted back to its usual path so no other part of MOS knows it exists. The key is sealed to the machine's TPM at first boot and the machine opens itself unattended; when the TPM refuses, the machine still boots and serves a page that asks for the recovery key. **That key is the backup recovery key** — one secret, one card to write down, written to the backup agent's key file before the agent has ever run — and it can now be replaced.

How the vault opens is a setting of the machine, with two modes. **Automatic** is the default: the chip releases the key to the machine itself, a power cut is invisible, and a thief who takes the whole machine and knows Linux gets in. **Password-protected startup** is one switch in settings: the chip is enrolled with `--tpm2-with-pin=yes` and the owner's Suite Manager password as the PIN, so after every restart the machine's page asks for that password before anything of the owner's opens. What MOS claims is written per mode and never as a blanket.

Reason: The owner asked for BitLocker: nothing to do day to day, and a stolen machine that gives up nothing. BitLocker's two halves are a TPM-sealed key and a login screen that cannot be bypassed because the boot chain is measured. The first half ports directly. The second is what does not, and the first version of this entry got the reason wrong. Full-disk encryption was rejected because its unlock lives in an initramfs, which on a headless machine means a console visit — still true. But the entry then argued that an offline edit of the plaintext system partition only "captures the next passphrase" and that "a burglar who takes the machine never comes back to use that". That assumed the owner types the passphrase. With chip-only unlock the chip types it, on the thief's first boot: mount the system partition on a laptop, add a unit or edit the vault agent, put the disk back, boot it, and every PCR still matches because shim, GRUB, kernel, initrd and `grub.cfg` are all unchanged. **On this layout a chip that opens the vault by itself opens it for whoever holds the machine.** Measuring more of the boot chain does not touch that, because the thing being edited is not measured at all. Only two things close it: a secret that is not on the machine, or encrypting the system half as well. The owner's password is the first, costs no new secret — someone holding the stolen machine who also knew that password could sign in and download everything anyway — and is checked by a chip that locks itself after a handful of tries, which is what makes an ordinary password a strong disk key.

Consequences:

- **The claim is made per mode, and the blanket version is never made.** Automatic: a disk pulled out, moved into another machine, or sold with the machine gives up nothing; the whole machine stolen and switched on opens itself. Password mode: the stolen machine gives up nothing, and neither do the backups whose credentials are inside the vault. Site copy, changelog and the settings screen all say it this way.
- **Password-protected startup is off by default, offered in settings, and never asked at onboarding.** With it on, a power cut leaves the apps down until a human types the password, which for an owner on holiday is days without their photos — a real cost, paid only by owners who choose it with the trade stated in one sentence beside the switch. Defaulting it on would lock out the owners it was never meant for; asking during setup would be one more decision at the moment they understand least.
- **The chip's secret is an opaque PIN to the vault agent.** `enrollTpm()`, `luksOpenWithTpm()`, the descriptor and the socket route never learn that it is the owner password; only Suite Manager's hook does. A later source for that secret — a passkey's WebAuthn PRF output, say — is another caller rather than a redesign, and an owner account always has a password for it to fall back to.
- **A password change teaches the chip first and commits second, and never refuses.** A password Suite Manager accepts while the disk still wants the previous one is a machine that signs its owner in and then locks them out after a power cut. If the chip will not take the new password the change still completes — an owner may be changing it because it leaked — the chip slot is wiped so the old password stops opening the disk, the state is reported as `needs-repair`, and the next sign-in, which holds the password for a moment, repairs it. If the vault agent cannot be reached at all, MOS cannot record that state and says so instead: the change completes and the screen tells the owner a restart may still want their previous password.
- **The recovery-key keyslot is never removed except by a deliberate rotation.** Every chip path — refused, locked out, wrong password, no chip, half-finished enrollment — fails into "ask for the key". No enrollment can brick a machine, and the worst outcome anywhere in this design is an owner typing their key once.
- **Re-sealing after a recovery-key unlock is authorised by the key, not by the boot state.** Someone who has the key already has everything, so re-enrolling the chip against whatever the machine now measures gives nothing away — and it is what makes one key entry the whole cost of a firmware update rather than the start of a habit. In password mode the page has no password, so it marks the slot for repair and the next sign-in finishes it.
- **PCR 4+7+9 with MOS-owned re-sealing was designed and dropped.** It does not close the easier attack above; its window was not one reboot, because MOS installs kernels automatically and never restarts the server on its own, so a loosened slot would sit open from install until the owner clicked Restart; it excluded PCR 8, leaving the GRUB console edit open, and including 8 would re-seal on cosmetic `grub.cfg` changes; and it coupled the vault to the patch agent through a transaction whose failure is a headless lockout after a routine patch. Kernel and bootloader measurement returns inside the encrypted-system-half work, where it protects something that cannot be edited.
- **Nothing is in `/etc/crypttab` or `/etc/fstab`, including the swapfile.** Either would turn a locked disk into a failed boot, which on a headless machine is a machine with no way left to ask for anything. `mos-vault.service` opens the vault instead, and every unit holding owner data requires it: while it has not succeeded there is no dockerd, no Suite Manager and no agent, so nothing can start against the empty directories the vault mounts over. In password mode the gate does not try the chip at all — it holds no password, so it would only fail slowly — and exits into the page.
- **Swap lives inside the vault.** On the plaintext partition it would be pages of decrypted app data lying next to the thing the vault protects. A machine with no vault keeps a `/swap.img` as before.
- **Caddy's TLS private key stays on the plaintext partition, deliberately.** It is what lets the unlock page be served over HTTPS on a locked machine, and a machine with a domain serves it that way. The cost is that a thief gets the TLS key for the owner's domain; certificates are short-lived and revocable, so this is the right trade, recorded rather than left as an accident. **The unlock page is served on every door the machine has, including the plain-HTTP ones**, which on a LAN-only machine with no domain means the recovery key and, in password mode, the owner's password are typed over HTTP on the owner's own network. That is deliberate and is the lesser of the two: the alternative is a machine with no way to be unlocked at all until its owner buys a domain, and the exposure is a listener already inside the network the suite is only reachable from. Onboarding had the same shape before it. What closes it is LAN HTTPS, not withholding the form.
- **The recovery key can be replaced, and the two halves rotate differently.** A rotation generates a new key — never owner-chosen, which is what protects the entropy and the checksum — rekeys the disk, re-keys every archive MOS can reach, and issues a new kit through the same component that showed the first one. The disk half is complete: LUKS wraps one master key per passphrase, the old one is removed, and reaching the master key needs the disk in hand. The archive half is not: restic has no master key rotation, so anyone who had the old key *and* a copy of the bucket can still read the snapshots that already existed. **That sentence is shown to the owner, unhedged, on the screen that rotates.** A destination that was not attached keeps the old key until it is next plugged in; superseded keys are kept for exactly that reason, and the engine re-keys each repository the first time it sees one.
- **Rotation is affordable here because MOS is the register.** Unmanaged rotation produces the pile of BitLocker printouts nobody can tell apart — which key opens which disk cannot be verified without the disk in front of you, so discarding is unverifiable and keeping is free. The current key is in the vault and on screen, so a rotation replaces one value instead of adding to a heap. It also takes the acknowledgement back: the kit in the drawer is wrong from that moment, so the gate that stood before the first backup stands again until the owner says they have the new one.
- **Creating a vault is refused until the machine is armed.** The published image is baked by running the same bootstrap inside a VM and snapshotting the disk, so a vault created there would be written to every download with one shared key. `mos-image-finalize` writes `/etc/mos/vault-armed` as its last act and refuses to finalize a bake that has vault state or a LUKS partition on it.
- **A takeover restore rekeys the disk, and never adds a key to the archive.** Adopting another server's key for the backups alone would leave an owner holding a card that opens their archive and not their server, found out at the one moment it matters. So the takeover points the vault at the entered key first — `luksAddKey`, prove it opens, `luksRemoveKey` — and only then writes the key file. A rekey that fails abandons the whole adoption and says what still opens what, because the restore itself succeeded. The rejected alternative was adding this machine's key to the archive instead: it was tried before this release and discarded on evidence — five test restores of one production bucket left five keys able to open it for good, and stale keys on an archive are a hazard that never expires. Startup protection is a property of the machine, not of the backup, so a restored replacement starts in automatic mode.
- **The key is handed over at setup, with the server login, on a page that holds Suite Manager shut — not at the first backup, and not on a card.** Once it opens the disk, an owner whose TPM refuses before they ever took a backup would be locked out of data they were never given the chance to protect; and a card on Home that shows one secret and silently drops the other is how the first hardware install handed over the login alone. One page after the terms shows both secrets, states their opposite lifecycles — the server login exists only there and is deleted on confirmation, the recovery key stays on the machine for good because unattended backups and the disk itself need it — and nothing else in Suite Manager is reachable until the owner confirms. `GET /api/setup/status` carries `handover` for a signed-in caller, `login` as `pending`/`unreadable`/`done` and `recoveryKey` as `pending`/`unknown`/`done`. `recoveryKey` is the vault agent's own `handover`, so the page asks exactly while the escrowed copy exists, and a later key rotation is Backup & Restore's to hand over rather than this page's; a read that fails is `unknown` and holds the page, never `done`, because a handover skipped while an agent was silent is a key the owner was never shown. The recovery key is acknowledged first, because that step is reversible and deleting the only copy of a password is not. The key block is a shared component so the handover page and Backup & Restore cannot drift, and the printed kit names both jobs and both startup modes.
- **The vault has a handover state, and it can only refuse to open once its owner holds the key.** `handover` is `pending` from the vault's creation until the owner confirms on the handover page that they have saved the recovery key, and `done` after. While pending, a copy of the key is escrowed root-only on the plaintext partition and opens the vault whatever the chip does: the key is first shown after sign-in, sign-in needs an open vault, and a machine whose chip refuses before then — no chip, a missing library, a firmware change — must still reach its owner. When the escrow opens a vault whose chip is unenrolled or refused, the same key authorises teaching the chip again right there, so the first restart after the handover opens on its own. The vault agent destroys the escrow (overwrite, then unlink) the moment the backup agent's record says the owner acknowledged the key, checked at agent start and before every status answer so an acknowledgement recorded while the agent was down still lands. Until then the encryption protects nothing and the handover page says so in those words; nothing behind that page is reachable while it is pending, and the startup-password route still refuses in that state as a server-side invariant, because a forgettable password must not become the only way in before the owner holds the other one. It is the same handover shape as the server login: made on the machine, kept until the owner confirms they have it, then removed.
- **One module owns the recovery key's files.** `system-agents/lib/recovery-key-store.cjs` holds the key file, the record of its acknowledgement, adoption and rotation, and the escrow, with their paths and modes; the vault agent and the backup agent both call it and neither knows the other's paths. The backup engine keeps only its own bookkeeping of superseded repository passwords beside the key.
- **Every unit MOS renders waits for the vault gate, on every machine, unless it is listed as vault-independent with the reason.** The gate exits 0 where there is nothing to open, so the requirement is never decided per machine and never conditional on reading the disk. The list is short — the gate, the unlock-page agent, the address banner, host keys, the self-installer, the bake finalizer — and `test/unit/vault-gating.test.cjs` holds the rule over the installer, the reconciler, the autoinstall seed, the image payload and the host-patching unit. The first hardware install found three units outside it: the login generator, which set a new server login on a locked boot; the post-patch health check, which wrote a false "everything is down" report to the plaintext side; and the reconciled agents, which the first update would have started against the empty directory.
- **The console tells one truth.** The server login is its own file under `/etc/issue.d`, written by a vault-gated generator after the address banner and removed when the owner confirms it, so it is on the screen for exactly as long as it matters and can be neither hidden by the banner's clear nor stacked by a re-run. The banner says "Locked" and where to enter the key when the gate failed, and "Installed and running" only when it did not. The installer stick shows no login of its own — it would be dead the moment the disk is written — so the disk's first boot is the first time a login exists. The login's file names and the installer-media marker are declared once, in `shared/`, one installer module renders the generator, the remover and their units for both seeds, and Suite Manager reads the same names.
- **An installed machine is born with its own identity, down to its disk identifiers.** The stick is running while it copies itself, so the copy carries the stick's SSH host keys, machine-id and journal; the installer clears them on the target and the disk's first boot generates its own, as a fresh install would. The copy also carries the stick's disk GUID, partition GUIDs and filesystem UUIDs, which is everything the firmware entry, GRUB's search and the kernel's `root=` find a disk by: with the stick left in they resolved to the internal disk, the stick booted the disk, and the installer saw a fixed root and never offered to reinstall. The installer therefore gives the copy new GUIDs and UUIDs and rewrites fstab and the GRUB configs to them, checking each file afterwards, because one reference left behind is a disk that boots only while the stick is in. Two machines installed from one stick share nothing, and a stick can always reinstall over the disk it made. The one identity that is deliberately not remade with it is how the machine asks for an address: netplan sets `dhcp-identifier: mac`, because the default derives that identifier from machine-id, so every reinstall would arrive at the router as a machine it has never seen and be given a new address — and the suite is published at a name that spells that address out. The MAC is unique per machine without being remade, so an owner who cannot open their router keeps their address anyway.
- **The disk is laid out in one place, and the table is fitted before it is read.** The image is written byte for byte, so its GPT still ends where the build's disk did; on a bigger disk the layout read that as "already full" and recorded the machine as unsupported, which the release verify caught on its first run. The gate's first act at creation is `sfdisk --relocate gpt-bak-std`, which moves the backup table to the end of the disk and sets the last usable sector to match, and only then does it plan. The installer copies bytes and clears identity and does nothing to the layout: the grow-to-fill it carried from before the vault never ran on the lab machine, and had it run the gate, which never shrinks, would have had nowhere to put the vault. A stick install and a direct write to the disk now go through the same first boot.
- **The descriptor records decisions, never state.** `/etc/mos/vault.json` names the device a vault was made on and how its chip is enrolled, or the reason no vault was made. Whether the vault is open is the agent's answer, read from the mapper.
- **Ubuntu 24.04 ships systemd's TPM2 support without the libraries it loads for it.** `systemd-cryptenroll`, `systemd-cryptsetup` and the LUKS token plugin are in the `systemd` package, but the three tss2 libraries they load on demand (`esys`, `mu`, `rc`) are only Suggests, and a server install was found with two of the three: enrollment failed with "TPM2 support is not installed" and the gate, which swallowed the tool's output, logged nothing. The installer installs the three by name together with `tpm2-tools`, and a first boot whose enrollment fails prints the tool's own reason.
- **MOS builds the backup tools and never enforces the practice, but always says which copies are within reach.** Immutability is enforced by whoever holds the storage — anything MOS can undo, ransomware with root can undo — so object lock is a bonus where a provider offers it and can never be the architecture. The universal answer is the drive that is not plugged in, so MOS remembers every drive it has backed up to by filesystem UUID, lists the ones that are away with how old the copy on them is, says per destination whether something that gets into the server could erase it, and tells an owner to unplug a drive it has just written to. None of it blocks anything. The one sentence MOS must never write is a promise of protection the owner does not have.
- **There is no migration for an existing install.** A machine that grew to fill its disk under the previous layout has no room for a vault and is left exactly as it is, with a sentence saying why. The route to an encrypted machine is a fresh install and a restore, which is drill-verified. A managed update still installs the gate and the agent everywhere, because the Caddyfile it upgrades gains the route the unlock page posts to and a route pointing at an agent nobody installed is worse than either end of it. What does not follow is the Docker requirement: it is written where a vault exists and removed where one does not, since on a machine with nothing to wait for it is only a new way for a working headless server to fail to come back from a reboot. Cloud installs stay unencrypted: MOS does not own the disk layout there, and a provider that can snapshot the running machine is not a threat encryption at rest answers.
- **The release pipeline proves encryption rather than assuming it.** The verify VM is given a virtual TPM — swtpm under QEMU, `Enable-VMTPM` under Hyper-V — so the sealed-unlock path runs on every tag, and `check-target.sh` fails the build unless the third partition is `crypto_LUKS`, the system partition stopped at its cap, and no swapfile sits on the plaintext disk.

## 2026-09-13: A Backup Restores Forward Only, And A Generation Break Keeps Its Last Reader Installable

Decision: A restore point is readable by the MOS release that wrote it and by every release after it. Restore never installs an older MOS and never migrates backward: it reuses the software already on the machine together with the backup's own app packages, which carry their own pinned image digests and `minimumMosVersion`. `RESTORE_COMPATIBLE_SCHEMA_VERSIONS` therefore only ever grows — raising the written schema appends, never replaces. Removing a version from that list is a generation break, a major platform event, and it carries one obligation in place of migration code: the last release able to read the retired format stays installable for good — tag, installer and image retained — and the refusal names that release from the manifest rather than from a constant.

Reason: The owner asked what happens when a server dies and its replacement is installed months later, several versions ahead. Pinning the replacement backward to the recorded version would mean standing up an unsupported platform on the machine that now holds the only copy of the data, then performing the same version jump anyway, at the worst possible moment. Forward is safe by construction instead: a package's `minimumMosVersion` is always satisfied by a later MOS, and Suite Manager's store already applies pending migrations on start. The real defect was documentation — the restore guide instructed an owner to install "a compatible version", which the hosted installer does not produce — and a warning that described forward restore as a feature not built yet. The second question behind it was whether a future MOS 2.0 would be locked in by migration debt. It is not: the escape hatch was already exercised once, when the unencrypted bundles of schema 2 and 3 were dropped and their refusal pointed at MOS 0.19. This entry makes that precedent the rule. Supersedes roadmap item AL7.

Consequences:

- **Two promises, and only one of them is permanent.** Readability — any archive can be opened by something the owner can still install — never breaks. Continuity — a live suite carries forward in place — is per generation and may break. Keeping them apart is what lets MOS be redesigned without holding old data hostage.
- **A generation break costs retention, not engineering.** No forward-migration code crosses the boundary. What crosses is a release that stays downloadable, which is a `RELEASING.md` obligation.
- **The floor is restic, not MOS.** A destination is a plain restic repository of real directories, so the recovery key and a restic binary return the files with no MOS involved. That is why a clean break is always allowed.
- **The unsupported direction is reported, not attempted.** A backup written by a newer MOS than the machine restoring it produces a warning naming both versions and telling the owner to update first; it is never half-applied.
- **MOS does not downgrade itself.** No version argument on the installer, no restore path that fetches an older tag. Both were considered and rejected as the thing that would create the lock.

## 2026-09-10: The Backup Screen Answers One Question, And Says Per Place Which Key Opens It

Decision: **Backup & Restore** is ordered by the questions a non-technical owner asks, in that order: a status banner carrying **Back up now** (am I safe, where does it go, when was the last one), then **Where backups go**, then the schedule, then restore points grouped under the place that holds them, then activity. A closed gate — an unsaved recovery key, an unfinished restore — replaces the banner rather than stacking above it, and never more than one at a time. Each destination is one row with three fixed zones: the radio that selects it, one status line that states its condition once, and at most one contextual button plus a **More** menu. A store written by another server carries a permanent line naming which keys open it, and the recovery-key row names, in a sentence, which places this server's key covers and which still need someone else's. The recovery-key record gains `adoptedAt` so the screen can tell "your key" from "their key, which is now yours". The screen lives in `suite-manager/frontend/src/features/backups/` as `model.ts` (every sentence the page can say, as pure functions over the agent status) plus the panels and dialogs that render them.

Reason: The page had grown feature by feature into six boxes of equal weight, and the owner said "I actually don't know what's going on myself here." Two failures were specific rather than cosmetic. Status was split across a pill, a status line, a helper line and a footer line, each true and none complete. And unlocking a bucket written by another server reported the result in one sentence that disappeared on reload, while the key panel at the top never said what it covered — so with two buckets and three drives there was no way to tell which key opened which, which is exactly the question a recovery depends on. Placement of the destination choice had drifted three times in one day by patching; the order above was designed once, against the owner's mother as the test, and is recorded here so it is not patched again. Keeping the sentences in `model.ts` rather than in JSX is what makes the copy reviewable without reading the layout, and the module replaceable in isolation.

Consequences:

- **Selection is the only choice.** No separate "make default" control; the picker beside **Back up now** appears only when more than one place is ready and never changes where automatic backups go. A selected place that is currently absent stays listed and stays selected, synthesised from the primary record if it is not in the drive list, so a drive in a drawer is never silently replaced by whichever drive is plugged in.
- **One truth per row.** The coloured dot carries severity; the status line carries the condition. A row never says the same thing twice in two shapes.
- **Which key opens what is a permanent property of the place**, not a message. It comes from the restore points themselves — a store whose points name another install wrote them is that server's — so it survives reloads without a new record on the destination.
- **Only kinds MOS can actually do are offered.** The add-destination flow lists a drive you plug in and S3-compatible storage; a network share and a "second internal disk" were designed but are not built, and offering them would be a promise the backend does not keep.
- **What the screen cannot answer, it does not claim.** A drive that is unplugged is simply not listed by the agent, so rows say "Not connected" without inventing a last-seen date, and a restore point carries no per-point check result because none is stored.

## 2026-09-09: Automatic Backups Have One Destination, And A MOS Update Backs Up There First

Decision: The destination MOS writes to when it backs up without being asked is a **primary destination**, owned by `system-agents/backup/primary.cjs` and chosen by selecting the destination's row under **Where backups go**. The schedule keeps only when it fires and how many copies it keeps. A backup the owner takes by hand goes to the selected place too, unless they pick another one for that backup from a picker that appears only when there is more than one to pick from. On top of that, a platform update begins with a backup of the whole suite, always: the update job gains a phase before `checking` — stage `taking-checkpoint`, and `waiting-for-backup-destination` while it waits — carried on the job record as `checkpoint: { backupId, jobId, status, target, waiting }`. The update agent takes it by calling the backup agent's own `POST /v1/backups` with `destinationId: 'primary'`, so the checkpoint is an ordinary backup and inherits every refusal already enforced there. There is no switch and no setting. The one moment a real choice exists is while the destination is away, and that is where it is offered: **Cancel update**, or **Update without a backup**. The two agents also now ask each other what they are doing: a scheduled backup waits while an update is running, and an update refuses to start while a backup, restore, check or delete is.

Reason: The clock went beside the backup queue in the 2026-09-06 decision so a scheduled backup would not be a second kind of backup; the checkpoint goes beside the update queue for the same reason, because the update agent is what applies the update and therefore what has to hold the apply open until a restore point exists. Suite Manager could not: it is unprivileged and it restarts partway through the apply. The primary destination is the Home Assistant shape: one place everything automatic goes, and any destination for a backup taken by hand — a drive for the fire safe stays one click. The first build of this had a switch under the Update button that started off when a backup under an hour old existed. It was removed for being unpredictable: a control whose starting position depends on the clock is a rule an owner has to learn, and the cost of a redundant backup is a few minutes. Asking only while the wait is real is the same principle the 2026-09-08 restore decision landed on.

Consequences:

- **One destination answers for every automatic trigger.** `primary-destination.json` holds it, `POST /v1/destinations/primary` sets it, and a schedule cannot be turned on before one exists. Selecting a row is the choice, the way Home Assistant marks a default backup location, so the list answers "where do my backups go" at a glance; the panel below it decides only when. The two are different questions and are kept in different sections on purpose. Earlier shapes — a select inside the schedule panel, then a button under each card — were rejected for making one choice look like two. A schedule set up before this release loses its destination and must be pointed again; nothing migrates.
- **A destination that is not there is a wait, not a failure**, exactly as it is for the schedule. The update holds at `waiting-for-backup-destination`, starts by itself when the drive or bucket is back, and offers the two answers. Cancel is honoured while the job is `queued`, `taking-checkpoint` or `waiting-for-backup-destination`; going on without a backup only while it is waiting, because a backup already running is left to finish. Past the apply's start, undoing is a rollback rather than a cancel.
- **The checkpoint is followed by its own job id**, never by "the current job": an owner who clicks **Back up** the moment the checkpoint ends must not fail the update. `GET /v1/jobs/<id>` on the backup agent exists for this.
- **A checkpoint that fails fails the update, before anything is fetched, built or reconciled.** The backup agent's own sentence becomes the update's, its logs go under the existing `on-failure` panel, and it is not retried: a refusal only the owner can clear — an unsaved recovery key, an interrupted restore — is not something to retry against. A queue that is briefly busy, or an agent mid-restart, is a wait instead.
- **No primary means no checkpoint, said out loud.** A fresh install with no drive and no bucket cannot wait for a place that does not exist, so the Updates screen warns above the button and the update runs without one. Refusing until the owner sets one up was rejected as an accept-terms-style gate on an ordinary flow.
- **Retention has one rule.** Manifests record `backup.initiator: 'update'` beside `owner` and `schedule`; `update` points are automatic and count against the one `keepLast` with `schedule` points, oldest first. A "newest checkpoint is never pruned" exception was built and removed: a pre-update backup loses its value within days of living on the new version, and the exception cost a second rule. The Backups screen labels the point "Before update to `<version>`".
- **Scope is the platform update only.** A per-app update transaction is a different thing with its own rollback, so the screen, the changelog and the site all say "before a MOS update", never "before updates".
- **The module boundary is three calls.** The update module uses the backup module's status, its cheap `GET /v1/summary` (current job and primary, without listing drives or reaching buckets), and "take a backup with this initiator". The backup module knows of updates only through a function injected into the scheduler at the agent's composition root, and one sentence. An agent that does not answer the summary is never treated as busy — a backup refused because a socket was missing would be a backup that never happened.

## 2026-09-08: A Restore Onto Another Machine Asks Whose Address Wins, Before It Touches Anything

**Superseded on 2026-09-20 in the part that matters: the question is no longer asked, `move` no longer exists, and the address is no longer something a restore decides at all.** See the 2026-09-20 entry for why — it locked an owner out of a restore that had succeeded. Both branches below are gone, including `copy`'s: a restore does not force TLS off, because a machine that was already serving a domain of its own keeps it. The rest of this entry still holds and is why it is kept: install identity, a restore point naming the machine that wrote it, no address inside app data ever being rewritten, servers staying unaware of each other, the dashboard tile following the applied domain, and the handover gate in front of the whole of Suite Manager.
...
- **A machine has an install identity, and a restore point names it.** `<agentStateDir>/install-id` is generated at the backup agent's first start, kept machine-local beside the engine key and never backed up. Manifests carry `source.installId` and `source.domain` (the applied base domain, or null), `GET /v1/status` reports `installId`, and "written by another machine" is decided by install id alone — a standby may carry the original's hostname on purpose, so the hostname is display only.
- **The question is asked exactly once, and only when it has an answer.** The same machine is never asked; a foreign backup known to carry no domain is never asked and needs only a notice that the address changed; a foreign backup with a domain cannot be restored without `move` or `copy`. Backups written before this release name no machine and no domain; they are not special-cased, so they restore as this machine's own, which is only wrong for the one pre-release server that exists. The check runs after the read-only validation and before the runtime is stopped, so a missing answer costs nothing.
...
- **Suite Manager as a whole waits for the handover, so nothing behind it waits on its own.** The console handover and its saved marker live in the Suite Manager state directory, which a backup carries whole and a restore replaces whole: a backup taken before the owner saved the login would ship this machine's server password, and a restore over it would delete the only copy. The handover page in front of Suite Manager is what prevents both; the per-route `409 SERVER_LOGIN_UNSAVED` refusals and the Backup & Restore pointer to a Home card are gone with the card. One gate in front of everything, not one per surface.
...
## 2026-09-07: The Recovery Key Is The Repository Password, The Owner Holds It, And Reading Someone Else’s Archive Never Changes It

Decision: The string in `RESTIC_PASSWORD` *is* the recovery key — no wrapping layer, no key-encryption key, nothing derived from the owner password. It is 140 bits of random material as `MOS-` plus eight groups of four Crockford base32 characters, the last group a 20-bit SHA-256 checksum, so a mistyped key is answered as a typo and never as a wrong key. One machine holds exactly one key, generated at agent start and kept at `<agentStateDir>/engine-key` mode `0600`, machine-local and never backed up. **A repository this machine did not write is read with the key of the server that did, held here and never added to the repository.**

Reason: Backups are encrypted with a key that lived only on the machine that wrote them. That is right for a stolen drive or a breached bucket and fatal for the one scenario off-site backup exists for — the server is gone and the bucket is all that is left. A key derived from the owner password would be worse than random: a server can rate-limit password guesses, a stolen bucket cannot.

Consequences:

- **Saving the key gates the first backup, and only the first.** On a machine with a vault the handover page in front of Suite Manager has already asked, so this gate stands where there is no vault, and again on every machine after a key rotation. `<agentStateDir>/recovery-key.json` records `acknowledgedAt`, a key fingerprint and `firstUsedAt`. Until it is acknowledged, `POST /v1/backups` and a `POST /v1/schedule` that enables the schedule refuse with a sentence and the screen opens the "Save your recovery key" dialog instead. Mounting a drive, connecting or testing a bucket, listing, validating, restoring and switching a schedule off are never gated: a replacement machine has to reach the surviving destination before it can enter the key that opens it, and it has no key of its own to save. The scheduler is not gated either — a machine migrating off the pre-release key keeps taking its nightly backups whether or not anyone has opened the screen.
- **The key can be shown again, behind the owner password.** "Shown once" is the default experience, not a security boundary — a signed-in owner already has root-equivalent power over the machine. The first showing asks for nothing; every later one verifies the owner password server-side and is not cached in the browser.
- **A wrong key is its own probe verdict.** `wrong-key` joins the probe causes and `locked` joins `absent` and `unreachable`. `openOrCreateRepository` returns a locked repository rather than throwing, a destination's `health()` reports `{ ready: false, locked: true, reason }`, and the row offers **Enter recovery key** instead of a failure. A locked repository is never cached, and never answered by creating a second one.
- **Unlocking writes nothing to the archive.** `POST /v1/destinations/unlock` checks the entered key against the destination and then keeps it in `<agentStateDir>/guest-keys.json`, `0600`, against that destination id; the destination folds it into its `repositorySpec()`, so every later command aimed there uses it and it is masked out of engine output. `POST /v1/destinations/forget-key` drops it again. A borrowed key never runs the pre-release migration, never creates an absent repository, and never counts as this machine’s `firstUsedAt`. Passwords travel in the environment, never in argv and never in a log line.
- **Becoming the other server is the only thing that changes a key, and it happens at restore.** A restore whose manifest was written by another install id and whose address plan is not `copy` calls `identity.assumeArchiveKey(destinationId)` after verification succeeds: the borrowed key becomes this machine’s own, `recovery-key.json` records `adoptedAt`, and the borrowed copy is dropped. That is the cold-standby path, and it is deliberately not at connect time — pointing MOS at an archive is not consent to alter it. A `copy` restore stays a second machine and keeps borrowing.
- **Rejected: adding this machine’s key to an archive it does not own.** `restic key add` is additive and re-wraps nothing, so it looked free. It is not: it puts a second password on someone else’s data, once per connecting machine and again after every rebuild of that machine, with no way to see or remove the extras from MOS. `keyAdd` and `keyRemove` remain on the engine for the pre-release migration of repositories this machine already owns.
...
- **The page is written by the installer and again on every reconcile**, unlike the Caddyfile beside it, which is written once because applying HTTPS replaces it. A machine updated from a release that predates the page would otherwise never receive the file its own Caddyfile points at, and a fresh install never runs the reconciler — so with only one writer a new machine served a bare `404` during a restore until its first managed update.
...
- **The durable backoff stores keyed digests, not the address or the email.** An unsalted SHA-256 of an IPv4 address is a 2^32 guess away, so the keys are HMACs under a per-install secret kept in `login-throttle.key` beside the database and never in it: the rows alone do not reverse. Deleting the file is safe — a new one is made and the old entries age out on the one-hour TTL — which is what makes this reversible. Entries are pruned on every write, so the table stays small and needs nothing scheduled.
- **A browser the owner has signed in from before skips the account-wide backoff.** That bucket is the one that cannot tell the owner from whoever is guessing their email: one address at two wrong attempts a minute kept the owner out from every address for 70–100% of the time, for as long as it kept going. A successful sign-in now sets a random-token cookie whose hash MOS keeps (the OWASP device-cookie pattern, the same shape as a session), and a sign-in carrying a known token is judged on the address bucket alone — so a stolen cookie is worth exactly what any single address gets, and never skips the password. A known browser's success clears only its own address, because clearing the account bucket would hand the guesser a fresh budget on every owner sign-in. Changing the password forgets every known browser except the one that changed it. A browser without the cookie — a new laptop, cleared cookies — waits exactly as before, which is the accepted cost: the alternative, scoping the account bucket per address, would give a guesser with many addresses many budgets.
- **A throttled sign-in tells the owner, by email, once a day.** The throttle stops the guessing; the message is what makes it visible before the owner next signs in. It carries counts and the server name, never an address, and the daily window is claimed before the relay is tried, so an attacker who can provoke a throttled attempt cannot make MOS open a relay connection at will or hammer a broken one. This is the first message MOS sends on its own behalf; the relay page's "MOS never uses it for itself" narrows to "never for anything but telling you about your own server".
...
- **The key still never leaves the machine, but the owner holds a copy of it.** This was the decision's sharp edge: offsite storage is exactly the case where the server is gone. Resolved 2026-09-07 by the recovery-key decision above — the owner is shown the key once, a replacement machine enters it against the same bucket, and the connect dialog says what the copy on the server does and does not protect.
...
Reason: The roadmap said systemd timer. It is the wrong home. An `OnCalendar=` schedule lives in a unit file, so every edit an owner makes to a setting becomes a system-file rewrite plus a `daemon-reload` in territory only reconciliation may touch. Worse, a timer firing an independent process would have to re-derive the things the agent already owns and enforces — the one-at-a-time job pipeline, mount liveness, the interrupted-restore block — or race them. Putting the clock beside the queue means a scheduled backup is not a second kind of backup: it inherits every refusal, and there is no path into the repository that skips the stop-and-quiesce sequence.

Suite Manager was never a candidate: it is the unprivileged component and the one that restarts during its own updates.

Consequences:

- **A missed window produces exactly one backup.** Due-ness is decided by comparing the most recent occurrence against `lastRunAt` and the moment the schedule was configured, not by counting elapsed time. A machine asleep at 3am backs up once shortly after it wakes; a machine off for a month does not wake up owing thirty. The `configuredAt` floor is what stops enabling a 3am schedule at noon from immediately running "today's".
- **A disconnected drive is a wait, not a failure.** The owed run stays owed until the drive returns or the next occurrence supersedes it, which is the ordinary case for a backup disk that lives in a drawer. Verified on the lab VM: the drive was unmounted before the window, the schedule reported waiting, and the backup ran within 30 s of it coming back.
- **The drive is identified by the repository on it, not by its mount path.** A reconnected drive can land on a different path; the repository id recorded with the schedule finds it. Drill-verified by remounting the lab drive at a different path and watching the owed backup run there.
- **Retention never deletes a backup an owner took.** It prunes only points whose manifest says `initiator: schedule`, oldest first, and only after a new backup has succeeded — so the suite is never left with fewer copies in exchange for nothing. A manifest without the field predates this feature and counts as the owner's, which is the answer that never deletes on a guess. Deletes go through the existing queued delete job, one at a time, because each rewrites the shared repository.
- Times are stored with an IANA zone captured from the browser that set the schedule, and preserved after that. A server in UTC and an owner who is not is the common case, and "back up at 3am" has to mean the owner's 3am. Opening MOS from a laptop in another country does not silently move the window; there is no control to change the zone afterwards short of clearing the schedule.

## 2026-09-06: The Backup Storage Engine Is restic, And It Is The Only One

Decision: MOS stores backups in a restic repository, pinned to 0.19.1 and installed into `/usr/local/libexec/mos/restic` by both host paths — the installer, which a machine that already exists never runs again, and reconciliation, which every managed update runs. Installing it in only one of them is the same rule-7 hazard as a unit written in only one: the first shape of this was reconciliation-only, and a freshly installed machine's first backup failed on a missing engine. Kopia is deleted — the module, the pinned release, the `MOS_BACKUP_ENGINE` environment variable, the `ENGINE_NAMES`/`engineNameFromEnv` selection, the base class the two implementations shared, and the comparison harness that chose between them. `system-agents/backup/engines/engine-restic.cjs` is now one file holding both the process plumbing and the CLI knowledge; `engine.cjs` keeps only the on-destination layout and a `createEngine()` that takes no engine name. Reconciliation removes a `kopia` binary it finds on a machine that carried the dual build. The engine name still travels in each destination's descriptor and each manifest, so a drive can always say what wrote it and a MOS that does not speak that format refuses it rather than writing into it.
...
The command line is never captured. A failure is described under a label the caller chose — `docker build for service "web"`, `npm run build:client` — and the runner does not know the argv exists. Output is masked twice, both times by exact value: the agent masks what it materialized itself (a service's secret-shaped environment values, the Cloudflare token) before the text leaves root, and Suite Manager's `redaction.cjs` masks it again against every secret it holds before the text reaches SQLite.

Reason: This amends a stated security property. `system-agents/README.md` promised that the HTTPS agent's errors "never include request values or command output", and the other agents ran everything with `stdio: 'ignore'` in the same spirit. The property protected the wrong thing. The hazard in an agent's error path is not what a command printed but what it was told: app containers are started with materialized secrets on their argv, so a single echoed command line leaks every secret of an app at once, while a build's stderr is the one thing an owner needs and almost never contains a secret. Treating output and arguments as one category meant the bundle carried every failure without its cause — a failed `docker build` was indistinguishable from any other failed `docker build` — on machines nobody can SSH into. Separating them keeps the real guarantee (no argv, ever) and drops the one that was costing the diagnosis. Masking by exact value on both sides, rather than by shape, is the same choice the logger and the bundle already made: the components involved hold the plaintext, so there is nothing to guess.

Consequences:

- The runner is the only way an agent runs a command, and unit tests on it prove that the argv never appears in a failure, that the attached tail is bounded, and that the rolling window is larger than the tail so a masked value cannot straddle its edge.
- The HTTPS agent's response contract changed: a failure now carries the agent's own `code`, `statusCode` and `details` where every failure was previously `HTTPS_AGENT_REQUEST_FAILED`/400, and Suite Manager forwards the code, so an owner sees `HTTPS_CADDY_VALIDATION_FAILED` or `HTTPS_RESTORE_FAILED` rather than one generic code. Both sides ship in the same managed update.
- The update job record gained `output`, set only on failure. Suite Manager caps it again at 20 000 characters on the way through `normalizeJob`, and the worker's full stream still reaches the transient unit's journal through the runner's `echo` mode.
- Update steps have a one-hour deadline. Previously a step that hung held the job open forever and blocked every later update; now the job fails with a reason that says which step did not finish. A command the runner stops is stopped together with the process group it started, because `npm run` and `docker build` do their work in children that inherit the output pipes, and signalling only the parent would leave the hang in place.
- `buildOperationDiagnostics` moved from `apps/` to `diagnostics/` because it now serves HTTPS failures as well as app operations; there is still one redaction implementation and one diagnostics shape.
- A Homepage apply failure's `details` are the output of `caddy` and `systemctl`, so the Customize screen keeps them under the shared disclosure rather than inline beside the validation sentences that were already there.
- Roadmap theme **I** is deleted: its gate — the bundle contains the reason for every failure class, including a privileged command that failed — is met.
...
- Roadmap **I2** was left unshipped here: the host agents still ran privileged commands with `stdio: 'ignore'`, so a package's failed `docker build` still discarded its reason. It amended a stated security property in `system-agents/README.md` and was argued before it was built; see the 2026-09-02 entry above.
...
- Confirmation is not authorization to apply an arbitrary previously downloaded directory. Suite Manager re-downloads the immutable candidate, repeats structural/privacy/compatibility comparison, and requires the resulting digest-pair token to match before asking the agent to stage it. SQLite records one active update operation per app with the expected installed digest, candidate digest, and latest durable stage; interrupted `candidate-staged` and `candidate-built` operations are therefore distinguishable from both an installed update and an unverified download after restart. The app agent builds all candidate images from its own re-verified candidate snapshot before any container, route, volume, or installed identity is changed. Build requests remain structured runtime projections, are bound to both installed and candidate digests, and are never persisted because their materialized environment can contain secrets.
- **The app agent's contract is one number, shared with Suite Manager and checked for equality.** Both ends ship in a single managed update, so a disagreement is that update applied half of itself (rule 7) rather than a tier to negotiate: `shared/app-agent-contract.cjs` holds the number, the agent reports it, and installing or updating an app is refused with a 503 naming both numbers. The update preview reports the same thing on the app card, and the diagnostics file names both numbers whether or not they agree. Nothing probes for capabilities; nothing tolerates a peer on another version in either direction. A package may still declare `update.minimumAppAgentVersion` and is refused when it exceeds the contract this MOS ships — that one is an external manifest contract and compares against the same constant. **MOS refuses to add an app through an agent it does not recognise, and never refuses to remove one.** Installing and updating are acquiring actions: refusing them costs the owner nothing they already had, and running them through an agent of unknown request shapes risks an app applied halfway. Starting, stopping, re-applying and uninstalling are containment actions — what an owner reaches for when something is already wrong — so none of them is gated. Uninstall in particular must stay available: removing a broken app and installing it fresh is a recovery path, and a handshake that has to succeed first would close it in exactly the state that needs it. An agent that genuinely cannot serve one of these fails it with its own message, on a screen already reporting a failure, which is strictly better than MOS refusing an operation the agent would have completed.
- **The update transaction the contract covers**, established over app-agent contract versions 3 to 6 and now stated as one shape: Suite Manager advances an update through candidate build, candidate activation, cross-app network reconciliation, conditional Homepage reconciliation, agent-owned snapshot promotion, and one SQLite identity/projection commit. Activation accepts only a pair of fully validated structured runtimes whose instance, package and installed digest identities agree, re-verifies both agent-owned snapshots, reuses deterministic named volumes, and changes the package Caddy block only after the candidate is healthy. Promotion uses same-filesystem renames, re-verifies both package digests, and retains exactly one prior snapshot only when the candidate declares rollback `safe`. Any failure after activation and before promotion asks the root agent to remove the candidate containers, restart and health-check the installed snapshot on the same named volumes, and restore its Caddy route; a rollback failure is reported separately from the failure that triggered it. Startup closes any operation left `running` and persists a recovery classification: pre-activation stages are `retry-safe`, post-activation `rollback-required`, post-promotion `commit-required`. An app that was not on Homepage stays absent; an applied stable-ID entry is atomically replaced from the candidate projection and restored if promotion or persistence later fails. Migration descriptions stay owner-visible instructions rather than executable hooks, named volumes and integration rows are preserved, displaced data is never deleted during an update, and undeclared structural breaks are blocked.
...
- Suite Manager and the app agent must be updated together, as with the earlier snapshot-root change: an agent on another contract version fails the install closed rather than falling back to a repository build.
...
- Updating an external app requires the app agent's current contract, the same as installing one.
...
## 2026-09-16: MOS Applies Ubuntu's Security Patches, Pinned To The Security Pocket, And Never Restarts The Server Itself

Decision: MOS installs and configures `unattended-upgrades` on every server it installs, with `Allowed-Origins` cleared and set to `${distro_id}:${distro_codename}-security` and nothing else, automatic reboot off. The definition lives in `infrastructure/host-patching.cjs` and is applied by both writers of host state — the installer at first boot and `reconcile-system.cjs` on every managed update. Host patch state is read through the read-only diagnostics agent as a fixed source and reported on the Updates screen beside MOS and app updates. The restart a kernel patch needs is offered there, confirmed with the shared acknowledgement checkbox, and performed by the privileged update agent; MOS never schedules one. A server where the owner already runs their own unattended-upgrades policy is detected before the package is installed, left untouched, and reported as owner-managed rather than as an error.

Reason: MOS updated itself and the apps and nothing updated Ubuntu, so an install running for months was quietly behind on kernel and TLS fixes while the Updates screen said everything was current. The published image makes that worse, not better: it is frozen at release time, so a months-old download starts behind on day one. Pinning to the security pocket is what makes this low-risk rather than a standing threat to the suite — Docker and containerd come from Docker's repository, Node from NodeSource, Caddy is a binary this repo builds, and every app runs in a container carrying its own userland, so everything that could plausibly break a suite is outside the pocket by construction. The kernel is deliberately not held back: it is the scariest package and also the one where being behind gets an owner owned, because container escapes are kernel bugs.

Consequences:

- **Rollback for a host that will not boot is the previous kernel, then reinstall-and-restore.** A kernel patch does nothing until reboot and Ubuntu keeps the previous kernel installed, so GRUB's Advanced options is the recovery path, and it is documented rather than built into a UI. `Remove-Unused-Kernel-Packages` stays on because Ubuntu's autoremove rules protect the running, newest and previous kernels, and a full `/boot` is its own way to wedge a machine. Beyond that, a wedged host is reinstall-and-restore: restore needs a working installed MOS, so the operating system is outside it.
- **MOS cannot learn from the fleet that a patch is breaking servers** — it has no telemetry, by promise. The project hears it from a person. That is why the post-patch health check exists (`mos-post-patch-check.service`, at every boot and after every unattended run, using the units the diagnostics agent already collects) and why its verdict is in the diagnostics bundle: one owner's report has to be enough to act on.
- **The answer reaches everyone through the signed advisory feed, not through a release.** `apps/advisories.json` gains an optional `host.heldPackages`, refreshed hourly and cached last-known-good like the rest of it. Suite Manager is only a courier: the update agent re-verifies the signature against `trust/official-catalog.pub` before writing an `Unattended-Upgrade::Package-Blacklist`, because a privileged write whose contents an unprivileged web app could name would be a way to stop a server taking security patches at all. It is empty on day one and expected to stay empty.
- **Major release upgrades stay out of scope.** MOS does not move an owner between Ubuntu releases, and there is no scaffolding for it. The docs name Ubuntu's support window for this release rather than promising patching forever.

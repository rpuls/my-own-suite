...
- **The key is handed over at setup, with the server login, not at the first backup.** Once it opens the disk, an owner whose TPM refuses before they ever took a backup would be locked out of data they were never given the chance to protect. One dialog now shows both secrets, and their opposite lifecycles are stated in it: the server login exists only there and is deleted on confirmation, while the recovery key stays on the machine for good because unattended backups and the disk itself need it. The recovery key is acknowledged first, because that step is reversible and deleting the only copy of a password is not. The key block is a shared component so the setup dialog and Backup & Restore cannot drift, and the printed kit names both jobs and both startup modes.
- **The vault has a handover state, and it can only refuse to open once its owner holds the key.** `handover` is `pending` from the vault's creation until the owner confirms on the dashboard that they have saved the recovery key, and `done` after. While pending, a copy of the key is escrowed root-only on the plaintext partition and opens the vault whatever the chip does: the key is first shown after sign-in, sign-in needs an open vault, and a machine whose chip refuses before then — no chip, a missing library, a firmware change — must still reach its owner. When the escrow opens a vault whose chip is unenrolled or refused, the same key authorises teaching the chip again right there, so the first restart after the handover opens on its own. The vault agent destroys the escrow (overwrite, then unlink) the moment the backup agent's record says the owner acknowledged the key, checked at agent start and before every status answer so an acknowledgement recorded while the agent was down still lands. Until then the encryption protects nothing, the settings panel and the handover dialog say so in those words, and password-protected startup cannot be turned on, because a forgettable password must not become the only way in before the owner holds the other one. It is the same handover shape as the server login: made on the machine, kept until the owner confirms they have it, then removed.
...
- **Backups and restores wait until this machine's server login is saved.** The console handover and its saved marker live in the Suite Manager state directory, which a backup carries whole and a restore replaces whole: a backup taken before the owner saved the login would ship this machine's server password, and a restore over it deletes the only copy while installing the other machine's "saved" marker. While a handover is pending or unreadable, `POST /api/backups/start`, `/restore` and `/schedule` answer `409 SERVER_LOGIN_UNSAVED` and the Backup & Restore screen shows only a pointer to the Home card. Installs that never had a handover are never waited on.
...
- **Saving the key gates the first backup, and only the first.** `<agentStateDir>/recovery-key.json` records `acknowledgedAt`, a key fingerprint and `firstUsedAt`. Until it is acknowledged, `POST /v1/backups` and a `POST /v1/schedule` that enables the schedule refuse with a sentence and the screen opens the "Save your recovery key" dialog instead. Mounting a drive, connecting or testing a bucket, listing, validating, restoring and switching a schedule off are never gated: a replacement machine has to reach the surviving destination before it can enter the key that opens it, and it has no key of its own to save. The scheduler is not gated either — a machine migrating off the pre-release key keeps taking its nightly backups whether or not anyone has opened the screen.

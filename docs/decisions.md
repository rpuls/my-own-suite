...
- **Caddy's TLS private key stays on the plaintext partition, deliberately.** It is what lets the unlock page be served over HTTPS on a locked machine, which is required: a recovery key, and now an owner password, must never be typed over plaintext HTTP. The cost is that a thief gets the TLS key for the owner's domain; certificates are short-lived and revocable, so this is the right trade, recorded rather than left as an accident.

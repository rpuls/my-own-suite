...
Decision: A restore decides nothing about addresses. A domain carried by a backup from another machine is **set aside** — kept in the restored settings as pending, with TLS off — and the suite is rebuilt on an address this machine can serve with no DNS and no credential. Serving a name is the domain module's job and happens nowhere else; the restore module may ask what address is served, never set one. Where a restored suite comes back is answered once, in `shared/served-address.cjs`, in one order: a domain that is being served, then the Easy Door, then the install-time name. The owner is told beforehand that the carried name will not follow, and afterwards what is still pointing at the machine they came from. **MOS reads DNS and never writes it.**
...
- **Recovery never requires a credential stored inside the thing being recovered.** The suite returns on an address needing no DNS and no certificate, so the owner reaches their password manager within minutes; and the domain provider credential is restored with the backup, so it does not have to be found at all. Every future recovery path is checked against this: an owner must never need something only the broken system holds.
...
- **A provider name is never a predicate outside the module that owns it.** `parkHttpsDomain` filtered on `tls_mode = 'cloudflare-dns01'`; a second provider would have matched no rows, set nothing aside, and reproduced this lockout exactly. The test everywhere else is now "a domain is being served". Module-specific vocabulary stays inside its module; other modules ask questions, not string comparisons.
- **The install-time name is the last resort for "where does this machine answer", not the first.** It describes where a machine was born, which on restored hardware can name the server being migrated away from — on the machine this entry came from, `home.mos.home` resolved to the old server. The Easy Door is preferred because it follows the live address.

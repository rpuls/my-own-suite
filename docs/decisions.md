...
- **One destination answers for every automatic trigger.** `primary-destination.json` holds it, `POST /v1/destinations/primary` sets it, and a schedule cannot be turned on before one exists. It is a property of the destination, chosen and marked on its card the way Home Assistant marks a default backup location, so the list answers "where do my automatic backups go" at a glance; the panel beside it is titled **Backup schedule** and decides only when. The two are different questions and are kept in different boxes on purpose. A schedule set up before this release loses its destination and must be pointed again; nothing migrates.

...
- **Unlocking adopts if this key is unused, and otherwise adds.** `POST /v1/destinations/unlock` checks the entered key against the destination first. A machine whose own key has never created or opened a repository replaces its key with the entered one and counts that as acknowledged — the cold-standby path, ending with exactly the key on the owner's paper. Any other machine runs `restic key add` with its own key, so both open the repository from then on, and the screen says which happened. Passwords travel in the environment and in `0600` files that are unlinked in `finally`, never in argv and never in a log line.

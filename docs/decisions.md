...
- **A machine with more than one internal disk is refused rather than guessed at.** Safe, and not yet an answer for the old desktop with an SSD and a spare HDD, which is exactly this theme's hardware. Roadmap **H4**.
...
- The install-time interaction changed shape rather than disappearing: the GRUB menu choice is replaced by the machine naming the disk it found and requiring the owner to type `YES`.

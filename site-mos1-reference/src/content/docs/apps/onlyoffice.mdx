...
import * as TechSpecs from '../../../reference/apps/onlyoffice.md';
import AppScreenshotGallery from '../../../components/docs/AppScreenshotGallery.astro';
import { onlyofficeHighlightsScreenshots, onlyofficeOverviewScreenshots } from '../../../data/app-screenshots';
...
<AppScreenshotGallery appName="ONLYOFFICE documents" items={onlyofficeOverviewScreenshots} showSecondary={false} />
...
<AppScreenshotGallery appName="ONLYOFFICE highlights" items={onlyofficeHighlightsScreenshots} showLead={false} />
...
> If you want the full story behind why ONLYOFFICE was chosen for this workflow, read [Goodbye Google Drive: My self-hosted Drive and Docs replacement](https://funkyton.com/goodbye-google-drive-my-self-hosted-drive-and-docs-replacement/).
>
> That article breaks it down even further: what I tested, why most self-hosted storage setups still fall short once browser-based editing matters, and why Seafile + ONLYOFFICE was the first combination that felt complete enough for daily use.

## Want a quick demo first?

If you want to test the browser-based editing workflow in isolation before committing to the full suite, you can use the standalone Seafile + ONLYOFFICE Railway template.

ONLYOFFICE is not a file-storage system on its own, so it needs something underneath it to store and serve the documents you work on. That is why the demo is bundled with Seafile instead of running as a standalone editor by itself.

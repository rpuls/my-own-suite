...
test('Hyper-V MOS V2 full platform regression', async ({ browser, page }) => {
  ...
  const dns01 = await test.step('apply DNS-01 HTTPS before post-DNS apps', async () => {
    ...
    if (!result?.homeUrl) throw new Error('Hyper-V full E2E requires DNS-01 before post-DNS app installs. Set MOS_V2_E2E_DNS01_BASE_DOMAIN and CLOUDFLARE_API_TOKEN in tests/e2e/.env.');
    if (result?.homeUrl) {

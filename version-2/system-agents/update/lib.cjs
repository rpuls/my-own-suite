...
const DEFAULT_BRANCH_REF = 'feat/app-platform-v2-lab';
...
function repoRootFrom(startDir = process.cwd()) {
  ...
  if (path.basename(resolved) === 'version-2' && fs.existsSync(path.join(resolved, 'package.json'))) {
...
function buildPaths(repoRoot = repoRootFrom(process.cwd()), stateRoot = process.env.MOS_V2_STATE_ROOT || '/var/lib/mos-v2') {
  ...
  return {
    ...
    stateRoot,
    updateStateDir,
    version2PackageJsonPath: path.join(repoRoot, 'version-2', 'package.json'),
    version2Root: path.join(repoRoot, 'version-2'),
  };
}
...
function trackLabel(track, ref) {
  ...
  if (ref === DEFAULT_BRANCH_REF) return 'V2 lab branch';
  return `Branch: ${ref}`;
}
...
function buildChangeSummary(paths, track, latestRelease) {
  ...
  if (track === 'branch') {
    return { items: extractChangelogSection(changelog, 'Unreleased'), source: 'CHANGELOG.md [Unreleased]', title: 'Upcoming V2 lab changes' };
  }
...
async function runApply(paths, { log = () => {} } = {}) {
  ...
  runNode(paths, path.join('version-2', 'scripts', 'reconcile-system.cjs'), [], log);

...
Your suite comes back on **this machine's own address** — the one you were using while you restored. That address needs no DNS and no certificate, so it works straight away: you can sign in, open your password manager and read everything in it.

The domain itself is set aside rather than moved, because a name points at one machine at a time. **Settings → HTTPS** names it and fills the form in with it, so applying it here is entering your DNS provider's API token and confirming.

Until that name points at this machine, anything set up against the old address keeps failing — phone apps, desktop sync clients, browser extensions. Three things fix that, and Backup & Restore says which one you are missing:
...
- **Give this machine the old server's address**, if the old server is off and both are on the same network. Nothing at your DNS provider has to change.

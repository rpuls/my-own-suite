...
The most recent backup you have should not be the one you last remembered to take. Under **Automatic backups** on the same screen, turn on **Back up automatically**, pick the drive, choose daily or weekly and a time, and MOS takes it from there.
...
- **A drive that is not connected is not a failure.** If the drive is unplugged when the time comes, MOS says it is waiting and backs up as soon as you reconnect it, up until the next scheduled time.
...
Mounted local/block storage only · no object-storage destination · encryption key held only on the server · whole-suite restore only · version-sensitive restore · no automatic backup before an update.

A way to save and re-enter the encryption key, a checkpoint taken automatically before every update, and object-storage destinations are the next things being built here. If you work with backup, storage, encryption, or recovery systems and want to shape how they land, [join us on Discord](https://discord.gg/YMpF6faBCv) or [open a GitHub issue](https://github.com/rpuls/my-own-suite/issues).

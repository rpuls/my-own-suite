...
The key becomes this machine's own only when you restore and choose to take the other server's place rather than restore as a copy. From then on one key opens everything, because the two servers are one. A copy stays a second machine with its own key, still borrowing the other's to read that archive. Either way, if the original server comes back on, both machines can write to the same destination; each restore point names the machine that wrote it.

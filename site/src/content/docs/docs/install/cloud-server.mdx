...
import DigitalOceanReferral from '../../../../components/DigitalOceanReferral.astro'
import VideoEmbed from '../../../../components/VideoEmbed.astro'
import installPoster from '../../../../assets/video/cloud-install-walkthrough.jpg'
...
<VideoEmbed
  videoId="f9Y9Mbys1GQ"
  title="Installing My Own Suite on a cloud server"
  poster={installPoster}
  caption="Recorded against an early beta, so a screen or two may have moved."
/>
...
1. **Create the server** in your provider's dashboard: image **Ubuntu 24.04 (LTS) x64**, size at least **2 vCPU / 4 GB RAM**.
   ...
   As a worked example, on DigitalOcean that is **Create → Droplets**, image **Ubuntu 24.04 (LTS) x64**, and a **Basic** shared-CPU droplet with 2 vCPU / 4 GB RAM (about $24/month). Other providers differ only in menu names. Don't overthink the size — you can resize to a bigger plan later if your suite grows (see [Good to know](#good-to-know)).
...
There is no setting for this in Suite Manager yet, and moving an existing install is not a supported operation — so decide before you install, not after. If your library is genuinely large, weigh the per-GB monthly cost against [running MOS on your own hardware](/docs/install/own-hardware/), where the disk is a one-time purchase.

## Backup destination

Attach a separate persistent block-storage volume through your provider, format and mount it on this server, then select that mounted volume in Suite Manager. A USB disk attached to your own computer is not visible to a rented server. You can also connect an S3-compatible bucket, which needs nothing mounted and can sit at a different company from the server. Provider snapshots are an additional provider-managed recovery layer, not a consistency-checked MOS backup. Use encrypted storage and read [Back up and restore](/docs/guides/backup-restore/) before storing data.
